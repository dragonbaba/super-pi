...
Provider-owned enrichment runs at every model ingress: shipped models, provider fetch callbacks, remote catalog fetches, offline catalog restoration, and the final OAuth/extension result. Catalog storage is revisioned raw provider data. Provider-supplied `capabilities`, `costKnown`, and thinking maps remain raw catalog facts across restart, while host-owned provenance, diagnostics, and enrichment markers are removed. `Models` is the sole normalization owner. Providers explicitly flag pre-revision runtime-profile caches for migration; only that migration path strips locally derived manifests before the current provider profiler rebuilds them.

OpenAI-compatible request assembly applies defaults, merges only non-structural generation keys from `samplingParams`, runs independent capability gates, then invokes `onPayload`. Messages/input/instructions/system fields, snake-case and camel-case tool controls, web-search controls, reasoning/thinking, previous-response continuation, strict-schema controls, and cache protocol aliases are never accepted from `samplingParams`. Chat and Responses share one capability-controlled reserved set and add only their protocol-specific fields. `onPayload` is the only explicit final wire override and may deliberately replace gated fields; effective dispatch observation records that final override without retaining original payload text.
