...
OpenAI-compatible request assembly applies defaults, merges only non-structural generation keys from `samplingParams`, runs independent capability gates, then invokes `onPayload`. Messages/input/instructions, tools and tool controls, reasoning/thinking, previous-response continuation, strict-schema controls, and cache protocol fields are never accepted from `samplingParams`. `onPayload` is the only explicit final wire override and may deliberately replace gated fields.
