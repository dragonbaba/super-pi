...
The focused regression `tests/session-clean-timeout-boundaries.test.ts` passes all 9 tests. It covers selector method output, Main/Alt frame delivery, 100 toggles, fixed detail bounds, timeout parsing, inner-command scanning, dangerous mutation preservation, redirections, dynamic values, Windows `timeout.exe`, and nesting.
