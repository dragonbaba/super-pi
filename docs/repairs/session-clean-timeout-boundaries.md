...
The immutable comparison point remains baseline `6699fb432a8be8bdf3572bcdc427de852f436dd1`; the published candidate began at `982c84da121aee54f9cd05b2ec1df9c52ce18e8f`. The post-publication review added bounded action paging for long Session histories, the raw Enter compatibility fallback, compact output-redirection scanning for `>>`, `1>`, and `2>>`, conservative handling of file-descriptor redirection, and focused regressions for those cases. The selector action index remains global while rendering only a fixed action viewport; redirection recognition remains mutation evidence and does not grant authorization. Full repository checks will be rerun for the final candidate SHA. The original `baseline-selector.ts` policy rejection and subsequent exact removal are retained as a brief historical limitation; the file is not recreated or removed again.
