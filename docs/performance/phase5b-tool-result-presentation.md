...
Rebuild selection keeps only a bounded 128-element array after the existing 256-candidate classification. It does not clear the shared presentation owner: final discoveries are re-admitted oldest-to-newest and an internal exact-resident, link-only touch moves each selected record to the eviction tail. The touch uses only `toolCallId` lookup plus canonical content-array identity, preserves validation/artifact/continuation state and all retained accounting, and performs no source scan, digest, allocation, or eviction. Unselected canonical records remain resident whenever the existing 128-entry/128-Mi-code-unit caps permit; normal admission alone performs necessary capacity eviction. The deterministic probes cover 127 selected plus one unrelated record, 128 selected plus one unrelated record, and a live admission, requiring owner eviction order to match the oldest UI discovery while the newest advertised continuation remains a resident hit with zero additional full-source scan. UI rebind also synchronizes the session-owned canonical index after runtime setup replaces the message array, so unchanged repeated rebuilds add zero index work and the first subsequent live result performs one append plus one lookup without a history rebuild.

### Non-blocking G2 backlog

- `D-G2C-AMBIGUOUS-BACKUP-CAP`: malformed histories with more than 256 reused or ambiguous V2 identities can yield fewer than 128 discoverability entries. Identity remains fail-closed, no invalid handle is published, canonical UI content remains visible, and provider projection is unaffected.
- `D-EXT-MESSAGE-END-IMMUTABLE-V2`: the compatible extension API allows synchronous in-place `message_end` mutation, so a handler that returns no replacement must conservatively trigger one final canonical UI refresh. A future versioned API may evaluate readonly events, replacement-only transforms, or an explicit unchanged disposition.
- Measuring repeated provider projection after resume when no continuation is ever requested remains a non-blocking optimization investigation. It does not change current correctness, capacity, or handle-lifecycle gates.
