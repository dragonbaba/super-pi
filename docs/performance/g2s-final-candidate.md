...
Goal: SUPER-PI-G2-ALPHA-STABILIZATION-ASTRA. PR https://github.com/dragonbaba/super-pi/pull/24. The final Ready-review addendum conditionally authorizes normal merge after its exact-head gates; Alpha manual validation remains pending.

Final micro-closeout from immutable Ready red `f8e9bbf7dd2e088be7d028a7b75f37c05a0a4831`: deferred-handler red `e5acd03`, shutdown ordering fix `2abc40d`, counter/tooling red `3f5b097`, correction `2abef4a`. Only interactive final ordering/first-error preservation and one diagnostic comparison change production. Matrix capacity validation precedes exclusive mkdir. The status ledger describes deterministic pending-handler state, exact counter output pins, same-path preflight regression, and `D-EXT-FINAL-SHUTDOWN-TIMEOUT-POLICY`. No lifecycle timeout, new performance campaign, provider change or frozen rendering implementation change. The final commit containing this document is the CI candidate; its exact SHA and CI links are recorded in PR metadata, not a later documentation commit.

Micro-closeout local verification: focused 134/134; check/build/diff check pass. One full npm-test invocation stopped at its stale runtime-before-stop source assertion; the corrected affected suite and remaining 22 files plus memory workspace then passed. This is a completed local test inventory with a corrected interruption, not a claim that the initial invocation exited zero. Exact-head Linux/Windows CI must pass the full command uninterrupted before Ready. Heavy evidence was not rerun.
...
Original quit red: TUI/ProcessTerminal disposal → runtime invalidation callback → resetExtensionUI/updateTerminalTitle → rejected disposed-terminal write. Final path closes the extension UI generation and disarms replacement callbacks, independently releases UI owners and restores/stops/disposes the terminal, then always joins the shared runtime owner. Terminal-local errors do not skip runtime cleanup; unbounded extension handlers do not retain raw terminal ownership. Normal success exits 0; disconnected output exits 129 after both ownership domains complete; the first real owner cleanup failure remains a failure. Repeated shutdown callers join the same operation. Live session replacement still resets/rebinds its UI.
