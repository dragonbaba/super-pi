...
### Subsequent footer and release checks

Clean `7229cd43513d3fa43a0b7d5e793ea66878161bb0` passed check, offline build, the 281-test probe (277 pass, four POSIX skips on Windows), and full npm test. Standalone regular/fullscreen each completed 100 lifetimes of 100,000 actual UI updates. All seven owner WeakRefs cleared every lifetime; total heap deltas were −4,761,456 and −4,644,352 bytes. Final ten samples contain plateaus followed by ~9–11 KiB steps; this is not a strict zero-slope claim.

Footer iterator red `deb9128` observes 5,003 iterator.next results for 5,000 messages plus metadata. `f62d88a4d20fc121395b521fa6a1dbfe15546244` uses an indexed traversal of the same single returned entry array: zero iterator protocol calls and identical displayed usage/name. It introduces no cache, snapshot or pool. `footer-iterator-matrix` contains 70 hash-verified processes (history + profile, five per configuration). Compared to the earlier `8c3f18d` group, mean root p95 at 120×40 is 9.488 ms (−1.88%) for regular/50k and 9.476 ms (+5.18%) for fullscreen/50k. Fullscreen/5k is 3.279 ms, +0.607 ms / +22.70%, with between-process CV .102. Results are mixed; no general CPU speedup or reliable whole-chain allocation percentage is claimed. Indexed traversal closes the deterministic iterator-work assertion without changing the O(history) scan.

Status callback red/profile head `1dffae7dbaabc5b11ce934428eca0893cbc3259d` observes three distinct comparator and sanitizer functions across three renders. A five-process, 16-status, 200-chunk L3 profile samples the exact anonymous callback source lines. `86cacbc1921e9f507ed0e86a9e6e2a911c698659` moves both callbacks to stable module functions and uses tuple indexing rather than destructuring iterators; output stays identical and both identity counts become one. Five after processes complete. Mean root p95 is 1.3533→1.2742 ms (−0.07908 ms), with 511→483 root frames across the five processes. Footer top-site sampled sums are 1,902,016→229,600 bytes; this is a censored top-15 attribution sum, not total allocation or a percentage acceptance gate. Reports `footer-status-before-1..5.log` / `footer-status-after-1..5.log` retain exact head, source line/column, all latency percentiles and counters. The old AST inventory's two footer status closures are therefore historical, not current unresolved callback sites.

Four real custom-footer shutdown cases now prove factory/dispose exactly once, owner release before terminal dispose, no late factory registration and collection of the footer while the closed mode remains referenced. Four actual CLI trusted/untrusted project cases use isolated trust stores and auto-discovered `.js` project extensions, verifying execute-once versus no execution, preserved decisions and normal terminal cleanup. No production trust changes. The raw-result manifest now pins all ten default mode sizes/digests and the decoded PNG digest; fixture content and ANSI size are unchanged.
