...
Filesystem count per ordinary exact hit: four realpath calls (cwd, target, revalidated target, with resolver behavior counted separately), one targeted stat, R_OK access plus the resolver's F_OK access. The explicit adapter calls realpath three times; the resolver's platform/path fallback behavior must be distinguished from a constant count. No disk content open/read, full-result serialization, file hash, ledger-owned result hash or retained source reference. The ledger reuses one stat-options object allocated at construction; no per-hit signal/options wrapper is introduced. Argument bytes are bounded and recorded; corpus key material was approximately 112 bytes/hit. G2 integrity bytes are reported separately below.
...
Classification for Candidate Review: B0 = stale/scope/permission/secret/public/cross-session error; B1 = contract/bounds/performance/lifecycle; C = narrow test/docs/diagnostic closeout; D = deferred adapter/optimization. The p95 miss performance finding remains B1 unless the final measurement establishes the permitted numerical gate. Windows native identity support and grep/find/LSP/git adapters are D and outside this change. One Candidate Review and at most one incremental closeout review are authorized. No operation-id, persistence, Harness v2 or Phase 7/8 work.
