...
Status: **G2 Alpha Manual Validation pending → G2S final Ready-review closeout (Draft) → G3–G10 unstarted**.
Gate: the final Ready-review closeout addendum conditionally authorizes Ready and normal merge after focused checks, one full local test run, exact-head Linux/Windows CI, clean worktrees, unchanged main, and resolved threads. No push after Ready, auto-merge, squash, rebase, branch deletion or manual acceptance. The historical authorization statements below are superseded by that addendum.

## Final Ready-review micro-closeout

PR #24 returned to Draft before changes. Immutable Ready-review red: `f8e9bbf7dd2e088be7d028a7b75f37c05a0a4831`; main remains `d5516ca39bfd7940f8bce76ea6aeb63616099383` at local preflight. No previous commit was rewritten.

- **G2S-B0-03 (Ready-review classification)**: final terminal restoration waited behind unbounded `session_shutdown`. This identifier is assigned by the latest addendum; the older active-provider finding using B0-03 below is historical. Red `e5acd03` uses a manually controlled handler without a 5.5-second sleep. All ten mode/error cases fail with terminal disposal zero. Fix `2abc40d` preserves synchronous UI detach, then disables theme sync, drains input, stops/disposes the terminal, and always attempts runtime disposal while preserving the first real failure. Stable UI facade observation accounts for fullscreen's existing renderer transfer. No runtime/extension contract or timeout changed.
- Pending handler after the fix: terminal dispose=1, logical mode stop/UI dispose=1 each, raw=false, input/resize listeners=0, session.dispose=0. Closed title/status/widget/footer/notify actions acquire no owner or post-disposal writes/renders. Releasing the handler gives shutdown emission/runtime dispose/session dispose=1 each; normal exit=0, actual disconnect status=129, component/runtime EIO retains original identity and first-error precedence.
- **C-G2S-01**: red `3f5b097`, correction `2abef4a`: one-unit retry uses `comparePositions(start, chunkEnd) < 0`. Pinned bytes/cursor/token result passes on old code; its counter assertion fails 0 versus 1, then passes with the single comparison change. No algorithm, budget, scan, hashing or format change.
- **C-G2S-02**: same red/fix commits: construct/validate complete matrix before metadata/output mkdir. Default `--runs 10` rejects 1650 > 1500 with no output/child/manifest. A corrected five-case selected suite reuses the same path successfully with subprocesses isolated by the test; no ANSI benchmark or large matrix runs. Error text explains suite-dependent run capacity.
- **D-EXT-FINAL-SHUTDOWN-TIMEOUT-POLICY**: extension shutdown remains unbounded unless configured. Terminal restoration now precedes that wait; arbitrary extension Promise cancellation is not invented.

Local evidence lives outside Git in `D:/RMProjects/Pi-g2s-evidence/20260906-g2s/closeout-*`. Focused recovery/runtime/replacement tests and deferred red/green are recorded there. Exact-head CI and final-main CI must be recorded in PR metadata after the final documentation commit; do not create another documentation-only commit after CI. No heavy performance matrix, profile, GC packet or provider measurement is authorized in this closeout. Prior performance evidence and its stated limitations remain frozen.

Local final validation: 134/134 focused tests pass (including the existing 35 recovery/normal-signal cases, runtime exactly-once, replacement, ten deferred cases, two C regressions, and adjacent async-owner tests). Check and offline build pass. `npm test` was invoked once; it stopped at the old source assertion requiring runtime-before-stop. After correcting that directly related assertion, the affected file passed and the remaining 22 test files plus memory workspace completed successfully using the same runner discovery order. No second full local campaign was run. Final `git diff --check` passes. Exact-head CI must independently run the full command uninterrupted.

The final gate remains conditional until all current checks and the automatic Ready review complete. On successful merge and exact final-main CI, update this ledger locally to **G2S Alpha Retest Gate — awaiting user manual validation**. User-configured startup and real GPT-6 Astra streaming are not certified by automation.
...
Closeout [P2 3943342536](https://github.com/dragonbaba/super-pi/pull/24#discussion_r3943342536) identifies composite TUI disposal as a remaining mixed-origin boundary: mounted-component release can itself throw EIO. Red `214c894` uses a real mounted RELEASE_COMPONENT_RENDER_CACHE hook and observes the failure being masked in both disconnected modes (connected controls reject correctly). Fix `bdaaabedbf355cb54456ab8f4505cbea41264df9` no longer suppresses errors from composite ui.dispose or transcript transfer. Existing TUI-internal terminal restoration handling is unchanged. No TUI/terminal production change or error-wrapper API was needed. Recovery suite is now 31 cases, plus four normal/signal quit cases: 35/35 pass. Full post-closeout verification remains required; no further Review is requested.

Exact `ea839584707d76992899227a1e311294c9a3374b` passed local check/build/probe/full npm test/source audit/diff check and both [Linux/Windows CI 34019266386](https://github.com/dragonbaba/super-pi/actions/runs/34019266386). Probe: 318 total, 314 pass, four Windows POSIX skips. Final five-process profile per mode plus five ANSI processes all exit 0 with 15 verified report hashes. Five-lifetime GC releases seven owner WeakRefs each cycle; regular net +451112 bytes, fullscreen -6015576. These values do not establish strict zero heap slope. Reports are `ea83958-validation/`, `ea83958-profile/`, `ea83958-profile-summary.json`. Candidate P1 reply 3943327090 and resolved thread PRRT_kwDOUC6EIs6fp1OF preserve the repair evidence. The following earlier counts/statuses are historical.

The expanded error audit also reproduces masked non-output failures and late UI reacquisition: red `e1df085` has eight failures; `6b294a2` / `8b9826b` distinguish runtime/footer EIO from a terminal-output EIO. The final design recognizes an expected disconnect only at terminal drain/progress/TUI restoration boundaries. Runtime, footer, subscription and other owner errors remain failures even when their code is EIO; the first owner failure is preserved. A closed/stopping mode's lifecycle rejection observer writes the error to plain stderr and sets exitCode=1 without calling UI methods. Recovery matrix is now 27/27, with real ProcessTerminal disposal and raw/listener restoration. The intermediate broad error-priority approach in `18357b8` is superseded by this explicit boundary classification.

Before this final error batch, exact `13471f9ec206382298e49e00b1e4045587e0f06a` passed check, offline build, Alpha probe (306/302/four Windows skips), full npm test and source audit. Linux and Windows both passed CI [34018291106](https://github.com/dragonbaba/super-pi/actions/runs/34018291106). Standalone regular/fullscreen 100-lifetime × 100,000-update GC/WeakRef runs completed with all seven owners collectible every cycle; reports are under `13471f9-validation/`. These results are historical exact-head evidence, not a claim that subsequent source edits have completed final CI. The `4d35140` CI run contains the obsolete source-order guard failure already repaired in `13471f9`.
