...
## Input scope follow-up on 216904e (2026-09-18)

The current scope explicitly cancels external image-file drag/drop from Explorer or the desktop into a terminal window. The production review found no Windows/OLE/COM bridge, system hook, file-drop receiver or terminal-host bypass to remove. The earlier image test label `drop` was inaccurate: it injected a complete bracketed local path into the normal text input path. Those four cases are now named `path-paste`; their attachment, routing, unsent-draft, history, and original-file assertions remain. The misleading startup hint `drop files to attach` is removed.

The shared terminal input chain remains intact. `TuiAltScreen` still owns SGR mouse press/motion/release handling for output selection, copy and scroll, and its Windows-only right-click callback still reaches the existing clipboard paste action. `Editor`/`CustomEditor` text editing, attachment selection/deletion, bracketed paste, clipboard image paste and explicit `/image` file input are unchanged. A new production-input regression selects and copies a line containing an image extension, a `file:` URI and a Windows path through `TuiAltScreen`; it observes ordinary OSC 52 text copy, no image reads/Workers/attachments/vision calls, and a focus-out cancellation with no copy. The codebase has no application-level selected-input-text drag-to-move implementation to restore; host-native behavior remains outside the CLI and is not claimed as desktop acceptance.

Current terminology is: `external-file-drop` cancelled and unsupported; `internal-text-drag` and `text-selection-drag` retained as compatibility requirements; `path-paste`, `bracketed-path-paste` and `clipboard-image-paste` retained. External drag/drop is not a remaining work item and no terminal “forbidden cursor” behavior is claimed to be controlled by Super Pi.

Scope follow-up validation: the affected complete files passed 255/255, `npm run check`, `npm run build:offline`, `npm run test:hot` and `git diff --check` passed, and full `npm test` passed with 1816 total, 1758 passed, 58 skipped and zero failures/cancellations. The existing image lifecycle, sampled input allocation, interactive-input and frame-queue benchmarks passed. No new listener, Promise, image I/O, Worker, clipboard probe, filesystem scan or hot-path allocation was added; the only production change is removal of the misleading startup hint. Native terminal-host text drag/move remains outside CLI control and is not claimed as desktop validation.

## Regex module contract follow-up on 29d7385 (2026-09-18)

The user's review found that the new image patterns were still constructed inside consumers despite the repository's dedicated-regex-module convention. A TypeScript AST audit of production additions across the feature identified sixteen new literal locations. Extending the existing `tui-hot-paths` consumer gate reproduced twenty-one locations across image attachments, clipboard image handling, permission rules and visual truncation (including five adjacent pre-existing declarations).

Image path, clipboard status and attachment display patterns now live in the lightweight `utils/image-input-regex.ts`, shared downward by core and clipboard utilities. Permission patterns use the existing extension `regex.ts`; visual truncation reuses the existing `TAB_PATTERN` from `utils/shell-regex.ts`. No extension dependency is imported by TUI/core. The identical image-suffix and network-path expressions share constants. Match/split patterns have no global/sticky state; global replacement patterns are used with literal replacement strings. Existing matching, rejection, control filtering, line breaks and permission scope semantics are preserved.

The existing AST gate now covers all four consumers. An additional AST check requires regex declarations in the affected dedicated modules to be top-level constants. A repeated/interleaved production-helper regression seeds replacement cursors and verifies they reset, while path predicates and MIME suffix extraction continue to match correctly. The audit includes the newly added untracked regex module before staging, not only `git diff`'s tracked files. This is convention and allocation-contract compliance, not a new performance feature or broad repository regex rewrite. Results are recorded separately under `clipboardRegexModuleRound`; no patch/manifest is generated for the committed delivery.

Final local validation: check/build:offline passed; four initial complete files 60/60; eight complete related files 242/242; test:hot 32/32; full npm test 1815 total, 1757 passed, 58 skipped, zero failures/cancellations. The final production-diff AST audit found seventeen pattern declarations, all in dedicated module constants, and zero violations. Both image lifecycle probe modes and the existing sampled visual-tail benchmark passed, run sequentially before full tests. Ordinary/receive/steady counters remain 0/0/0 → 2/1/2 → 2/1/2; no remaining draft/editor IDs and 2/2 observed JS weak references released. Visual tail returned 10 lines with 9991 skipped and zero empty-owner microtasks/pending text. These are absolute regression observations, not a new percentage improvement claim. Native desktop/online acceptance and retained-directory limitations remain unchanged.
...
The failed check was `verify-linux` in run `35257440639`; `verify-windows` passed. Linux failed three cases in `image-placeholder-input.test.ts`: both custom-editor model configurations and the plain-editor fallback. Their shared mistake was feeding Windows-only right-button SGR input into Linux and expecting clipboard handling. Production deliberately gates that gesture on `win32`. Cross-platform ownership tests now call the installed host callback on other platforms; Windows keeps the actual mouse-input dispatcher path. The eight dedicated Windows gesture cases retain their platform skip and their assertions. This does not add Linux native right-click support or claim native desktop acceptance.

| Review | Root cause | Fix and evidence |
| --- | --- | --- |
| [4040155047](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4040155047) | Clipboard text-path fallback starts a second asynchronous file/Worker operation, but cleanup only knows the already-removed clipboard record. | Pass the original ownership predicate through `addFiles` and `finish`; recheck after reads, before publishing ready state, and after the nested await. A bounded per-operation list tracks only its own records for invalid-owner cleanup. Tests change focus, replace the editor and clear the draft during actual file reads and actual Worker decoding; unrelated attachments and new text survive, stale records never publish ready state, and original files remain unchanged. |
| [4040155056](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4040155056) | The filename sanitizer removes every C0 control, including line breaks in multiline auxiliary descriptions. | Keep filename labels unchanged. A multiline description sanitizer normalizes CRLF/CR to LF and filters terminal/C1/bidi controls while retaining line breaks. The real auxiliary extension receives controlled multiline/OCR-like output, persists it, and both live and disk-restored entry rendering preserve separate lines without executing injected terminal controls. |
| [4040155065](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4040155065) | A completed provider request leaves `_imageRequest` attached to the session; unrelated compaction/branch summary calls reuse the SDK stream function and encounter the stale model/policy guard. | Release the guard at assistant-message completion and agent-end, before extension callbacks. All preparation/context/header/payload checks remain in place while the request is active. Real SDK tests cover compaction and branch summarization after model or policy changes, in both model configurations and after successful or failed main requests. Existing invalidation tests now also assert guard release after failure/cancellation; new header-stage cases exercise the SDK callback before provider serialization. |

The offline transport fixture now consumes `transformHeaders`, matching the ModelRuntime boundary that the earlier direct provider fixture omitted. An initial attempt to exercise that hook waited because the old fixture never invoked it; the test process was stopped by its verified PID and the fixture was corrected. Two initial compaction-policy fixtures also needed their small compaction window reapplied after settings mutation; production limits and safety checks were unchanged. These diagnostic attempts are not reported as completed passing regressions.

The source/allocation audit covers the existing input → clipboard → file read → Worker → draft projection path and provider preparation → SDK checks → assistant completion → compaction/summary path. Ownership tracking allocates at most eight borrowed record references only for the optional clipboard batch, with release when that operation settles. Ordinary file input does not allocate that tracking array; keys, frames and provider deltas acquire no new Promise/controller state. Worker architecture, read-slot retention, count/byte/pixel limits, digest reuse, ask_user and permission checks are unchanged. The existing lifecycle and sampled editor-input benchmarks are used as regression evidence, not a new performance feature or speedup claim.

Native desktop and real online service acceptance remain unexecuted. No paid model or private screenshot was used. The original eleven policy-retained directories remain untouched. One additional directory from the interrupted header fixture, `C:/Users/Administrator/AppData/Local/Temp/sp-policy-boundary-8ZAQZ6`, was identified by the test's creation time/path; cleanup was rejected by automatic approval (`blocked by policy`). It is retained separately and no alternate cleanup tool was attempted. This is distinct from Worker/process lifecycle cleanup, which the completed regressions verify.

Final local validation for this round: static check, offline build, six full related files (207/207), hot source gates (31/31), and full `npm test` all passed. Full totals: 1813 tests, 1755 passed, 58 skipped, zero failures/cancellations. This includes 28 additional cases. Both existing lifecycle benchmark modes completed: ordinary reads/encodes/projections 0/0/0, reception 2/1/2, unchanged during subsequent edits, zero remaining draft records/editor IDs, and 2/2 observed JS weak references released under diagnostic GC. These results do not assert exact native peak release. `clipboardCiReviewRound` records commands, observations and log checksums independently of previous candidates; fresh published CI must be checked for the new commit.

## Clipboard review follow-up on afc11df (2026-09-18)

This round addresses the seven review comments posted against `afc11dfecd3d886e0cfea05e62fc23646bcf4a4d`. That published commit passed Linux and Windows CI; the new findings were review defects, not a new CI failure. Earlier sections and measurement entries are historical snapshots. Their uncommitted/CI statements apply to the candidate they describe, not automatically to this follow-up.

| Review | Reproduction / cause | Change and regression |
| --- | --- | --- |
| [4038981728](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4038981728) | An extension installs a separate `CustomEditor`; only the hidden default editor receives attachment IDs/text. | Project IDs, selection, removal and paste callbacks into the active compatible editor; release the inactive projection on replacement. Other editor implementations use a host-owned text fallback outside the replaceable dialog area. Test the real extension command/UI API, both model configurations, deletion, editor restoration, wire dispatch and canonical attachment identity. |
| [4038981737](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4038981737) | The preparing image record disappears before the asynchronous text fallback finishes. | Keep ordinary Enter and follow-up blocked for the complete clipboard operation. Test with and without an existing image; the first Enter preserves text, completion does not send, and a second explicit Enter dispatches. |
| [4038981747](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4038981747) | Right-click captures the editor but ignores a focus change while awaiting the helper. | Bind completion to editor, focus, draft, session, lifecycle and cancellation. Discard that operation's record if it becomes invalid. Test delayed image and text responses while another component owns focus; no text/image is inserted and no model is called. |
| [4038981752](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4038981752) | Clipboard and file batches race the single decoding slot. | Reject competing entry operations visibly; keep the slot occupied until read/decode cleanup settles. File paste and `/image` share the clipboard guard. A local path returned by clipboard text fallback remains within its already-admitted operation. |
| [4038981761](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4038981761) | `/image-remove` splices the draft record but leaves the clipboard helper alive. | Route the compatibility command through the same abort-aware ID removal as Backspace/Delete. Test observed cancellation, settled pending state and a successful subsequent paste. |
| [4038981774](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4038981774) | Alt+Enter transfers image ownership before recognizing an extension command, which can return success without accepting a message. | Classify registered extension commands before image submission, preserving draft identity and normal follow-up semantics. Test actual Alt+Enter dispatch during a gated SDK request, plus the compaction admission condition. No queue entry or recovery owner is created by the command. |
| [4038981782](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4038981782) | Public submission metadata accepts empty, non-string or duplicate attachment IDs. | Reject invalid supplied IDs at `snapshotImageSubmission` before SDK message admission; unsupplied identities are minted per slot, independently of decoded-content receipts. Test empty/whitespace/number/null/duplicate IDs through SDK prompt with no provider request or canonical user message. |

The pre-fix run of the expanded placeholder file reproduced all seven categories: 15 existing cases passed and 15 reproduction cases failed. Two additional coverage cases exercise a plain extension editor/dialog restoration and clipboard path fallback. Clipboard transport is controlled, while the ProcessTerminal dispatcher, host owner, local decoder, extension registration, SDK and provider serialization are real. The compaction branch test deliberately controls the `isCompacting` accessor while a real SDK request is gated; it is not a claim that this new case drives a complete compaction lifecycle. The existing full compaction regression files are still run. The older method-level `interactive-control-images` fixture now explicitly gives its editor focus, matching the `init()` step it intentionally omits; its assertions are unchanged.

The first full-suite attempt stopped at two old `tui-async-owner-closeout` fixture/source-contract assumptions: a missing focused renderer and two literal inline generation comparisons. The method fixture now provides its focused editor, and the source gate verifies the combined lifecycle/draft/session/cancel/editor/focus predicate, both post-await guards and invalid-owner cleanup. The one-controller budget and no-Promise-tail assertions are retained. The complete six related files then passed 179/179, including all 62 async-owner cases.

No image capacity, allocation budget or safety assertion was relaxed. The Worker decoder, immutable content receipts, auxiliary routing, ask_user boundary and final invocation authorization are retained. Rendering adds one stable, cached host Text component for incompatible editors; compatible editors continue to use their own attachment projection. New asynchronous state is allocated only at explicit paste/add/submit boundaries, not per ordinary key or frame. The audited path is ProcessTerminal input → active editor → clipboard/file admission → draft owner/Worker completion → attachment projection → existing layout/frame queue, including cancellation and editor replacement.

The lifecycle probe measured ordinary reads/encodes/projections of 0/0/0; reception 2/1/2; and no further work during 1000 attached text edits. It released all draft/editor IDs and both observed JS weak references under diagnostic GC. This is not a native-resource peak proof. The independent natural scheduling benchmark completed 14 scenarios: the maximum planned-to-accepted-input delay was 12.6364 ms and maximum planned-to-matching-frame write completion was 28.2184 ms, below the declared controlled targets of 100/150 ms. Ten probes were scheduled during actual Worker decode intervals; the other four covered header/size rejection. Peak process RSS high-water mark across these runs was 482.53125 MiB. Sampled cursor/editor-render workloads measured 33,849,112 bytes without an image and 50,437,192 bytes with one image over 2000 iterations; these samples are not a general UI latency or improvement claim.

Verification and bounded lifecycle/response observations for this candidate are recorded separately under `clipboardReviewRound` in `docs/performance/interactive-image-closeout-measurements.json`. Measurements use non-sensitive fixtures and simulated terminal sinks. Native desktop clipboard acceptance and real online services remain unexecuted; the user's prior screenshot proves the observed unsent marker only. The eleven previously retained directories remain an independent, untouched cleanup item. No performance credit is attributed to the unchanged drain or to a general TUI speedup; the old 20.4% figure remains specific to its visual-line sampling fixture.

Final verification for the afc11df review follow-up: `npm run check`, `npm run build:offline`, `git diff --check`, all six complete related files (179/179), `npm run test:hot` (31/31), and the three benchmark commands passed. The rerun of `npm test` completed with 1785 tests: 1727 passed, 58 skipped, zero failures/cancellations. These results include 17 new regression cases and the updated source/lifecycle fixtures. Published CI for afc11df is not reused as the candidate's CI result.

## Clipboard and placeholder follow-up after 4e783b2

The user reported a visible unsent screenshot attachment in super-pi, but Explorer drag/drop showed a prohibited cursor. They subsequently reported that the updated local OMP also appeared unable to accept that drag, and asked to focus on clipboard input and deleting image placeholders instead of a list of image commands. Local inspection identified OMP 18.2.4 (previously 18.0.0). This follow-up is an uncommitted change on top of 4e783b2; the published CI/review results below apply to that published commit, not automatically to this candidate.

- The draft's visible image placeholders are keyboard-editable units before the text. At the beginning of the text, Left selects the last image; Left/Right move between image placeholders and back to text. Backspace/Delete remove the selected image. Backspace at the beginning of text directly removes the adjacent last image, including image-only drafts. Typing returns to text; programmatic text replacement clears image selection. Existing text is not rewritten when an image is removed.
- Selection contains only a stable attachment ID. The editor keeps at most the draft's eight IDs, with no copied image data or full-text ownership scanner. Removing a placeholder calls the existing draft owner; a preparing clipboard operation is aborted and its late result cannot recreate the deleted item. Typed/copied display labels are ordinary text. Deletion releases the image; text undo does not resurrect a removed attachment. Re-add the source to restore it.
- The persistent command bar is replaced by a short hint using the effective cursor/delete bindings. `/image`, `/image-remove` and `/image-clear` remain compatibility/explicit-file-entry paths; users do not need them for normal clipboard addition and deletion. Failed-submission recovery/cancellation remains independently reachable. There is no external file-drop hint or receiver.
- Windows defaults now include Ctrl+V and Alt+V, while explicit user bindings still override them. An empty complete bracketed paste invokes the same clipboard action. The fullscreen renderer's existing right-click callback uses that action when the main editor is focused; other focused prompts retain their text-only paste path. Terminal-managed right-click/Ctrl+V may arrive as a pasted path or empty bracketed paste. No global right-click hook or Windows drag/drop bypass was added.
- A paste starts local preparation only. An Enter received while preparation is incomplete is still rejected with the existing retry-send notice; it is never replayed when preparation completes. Main/auxiliary model routing, submission snapshots, worker decoding, recovery, policy checks and queue limits are unchanged.

`tests/image-placeholder-input.test.ts` drives actual ProcessTerminal input, the editor, host owner, SDK and offline provider serialization in regular/fullscreen modes and both model configurations. The Windows gesture cases replace only the clipboard helper's child-process response and remain explicitly simulated clipboard tests. They are not native desktop acceptance. Native Ctrl+V/right-click and complete two-configuration submission/history acceptance for this candidate are still unexecuted; no online service was called. External file drag/drop is cancelled and unsupported; the retained path-input cases are bracketed/path paste, not native drag acceptance.

Validation commands and measurements for this follow-up are recorded separately under `clipboardPlaceholderRound` in `docs/performance/interactive-image-closeout-measurements.json`. The optional `--sample-input` flag on `scripts/bench/image-draft-lifecycle.ts` samples cursor handling plus editor rendering after initialization, separately for no image and one ready image. These sampled timings are not natural-frame or desktop display latency. This is correctness verification, with no new performance improvement claim. The eleven previously retained directories were not touched.

Final local verification for this follow-up: `npm run check`, `npm run build:offline`, and `git diff --check` passed. The five complete related test files passed 100/100; `npm run test:hot` passed 31/31; `npm test` completed with 1768 tests, 1710 passed, 58 skipped, zero failed/cancelled. The 15 newly added cases are included in these results. Both lifecycle benchmark commands above exited successfully. Unsampled counts match the baseline (ordinary edits: 0 reads/encodes/projections; receive: 2/1/2; later edits: unchanged). All draft records and editor IDs were released; two observed JS weak references cleared under diagnostic GC. Sampled candidate-only runs used 34,752,864 bytes over 2000 no-image cursor/render iterations and 49,312,464 bytes over 2000 one-image iterations. These are absolute workload samples, not a before/after improvement or a zero-allocation claim.
...
4. Use the retained explicit `/image` path entry only when testing a deliberate local-file add; do not call it drag/drop acceptance. Add the second fixture, inspect both unsent markers and remove one using `/image-remove N`.
...
## Follow-up GitHub review (reviewed commit f88a87f)

The first review-fix commit `f88a87fe881495e040008a603a373ea97a7886a7` passed both Windows and Linux in [CI run 35225935971](https://github.com/dragonbaba/super-pi/actions/runs/35225935971). Its Codex follow-up identified four additional boundaries. These changes retain the prior nine fixes; the previous `githubReviewRound` data describe f88a87f, not this follow-up candidate.

| Finding | Reproduction and correction |
| --- | --- |
| [4037331070](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4037331070): compaction retry outruns input admission | A real SDK overflow response enters automatic compaction while a TUI image submission is queued. An asynchronous ordinary input hook then delays admission. The old event notification launched flushing without awaiting it, and the retry requested stale context. An opt-in `criticalCompactionEnd` subscription now waits for the existing TUI owner at this retry boundary. Ordinary event delivery and noncritical listeners retain their behavior. Failed/cancelled admission pauses continuation and preserves unaccepted items; aborted/error compaction notifications do not auto-flush. Normal and cancellation regressions drive the real overflow/compaction/continuation path, with offline ModelRuntime responses and a simulated terminal. |
| [4037331082](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4037331082): Windows image probe failure blocks text | An unavailable/denied/timed-out PowerShell image helper rejected before the TUI text fallback. Ordinary image-probe failures now report their cause and allow the text path; a failed Windows text helper can use the existing native text reader. Abort and image quota rejection remain errors. Bounded stderr is preferred over a long encoded command when reporting the helper failure. Tests cover ENOENT/EACCES/timeout, abort, quota stderr, and Windows Alt+V dispatch through actual TUI/owner/text fallback with controlled child-process output. This last test is Windows-only and is NOT native desktop clipboard acceptance. |
| [4037331096](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4037331096): legacy rule scope is misrepresented | Restored legacy rules intentionally lack a cwd constraint, but `/permissions rules` displayed the primary directory. The registered command now displays `cwd=unscoped (legacy)` for those rules. Creation, matching, revocation and final authorization semantics are unchanged. |
| [4037331111](https://github.com/dragonbaba/super-pi/pull/38#discussion_r4037331111): rejected prompt overwrites an active system prompt | A waits in `before_agent_start`, B acquires Agent ownership, and A then writes its override before admission rejects. System prompt state now remains local until the existing Agent admission callback; rejected prompts never write shared prompt state. Real SDK regression checks both the admitted run and its subsequent tool turn use B's system prompt. |

Seven new reproduction cases failed on f88a87f before these repairs. Additional controls verify the helper abort/quota distinction and Windows TUI text fallback. They extend the existing regression files rather than creating another source tree. Native Windows screenshot/Explorer acceptance and online models remain unexecuted. The previously retained 11 temporary directories remain untouched; no cleanup policy was bypassed.

### Follow-up candidate validation

Ten additional tests cover the four follow-up findings and controls. Final source passed `npm run check`, `npm run build:offline`, `npm run test:hot` (31 passed), `npm test` (1,749 total / 1,691 passed / 58 skipped / 0 failed or cancelled), and `npm run alpha:g2-probe` (318 passed / 4 skipped). Twelve complete related files passed 375 tests with 21 platform/opt-in skips:

```powershell
node --experimental-strip-types --test tests/github-review-regressions.test.ts tests/image-review-boundaries.test.ts tests/image-acceptance-closeout.test.ts tests/interactive-control-images.test.ts tests/permission-interaction.test.ts tests/tui-async-owner-closeout.test.ts tests/visual-tail.test.ts tests/alpha-cli.test.ts tests/tui-retained-session-rebuild.test.ts tests/tui-frame-queue.test.ts tests/evidence-ledger-session.test.ts tests/alpha-compaction-quit.test.ts
```

Natural response, lifecycle and separate unsampled/sampled visual-tail baseline/candidate benchmarks used the same commands printed above, after tests completed. This repeat retains all capacity limits and prior assertions.

All 14 natural runs met the declared 100/150-ms state/frame targets: maximum planned-to-accepted 13.728 ms and matching-frame completion 28.921 ms. Near-24MP peak RSS was 484.5 MiB. The lifecycle probe again observed 0/0/0 ordinary input reads/encodes/projections, 2/1/2 on addition and no additional work during 1,000 attached edits/renders; cancellation at read start was 0.389 ms, both JS WeakRefs released and records were zero. These are simulated sink and diagnostic release observations with the limits above.
...
## Windows native clipboard cancellation follow-up (0936b42 follow-up)

The Windows image path remains ordered as PowerShell STA helper → native clipboard fallback → ordinary text/path fallback. The native fallback is supplemental; the test name and documentation do not call it “preferred native” and do not claim that an injected PNG fixture proves DIB/DIBV5 conversion.

The locked dependency is `@mariozechner/clipboard@0.3.9`. Its Windows implementation checks image availability through a synchronous `has_image` N-API call and exposes `get_image_binary` as an async JavaScript API. The inspected implementation creates a clipboard context, reads PNG/CF_DIBV5/CF_DIB and converts the result to PNG, but exposes no AbortSignal, deadline or stop operation. The Promise type alone therefore cannot establish timely cancellation. A controlled production-chain probe reproduced the application gap: after the PowerShell helper returned empty and native `getImageBinary()` started, aborting the caller left `readClipboardImage()` pending until the bottom provider eventually returned.

Windows native fallback reads now run in one short lived, application-owned child process only when the fallback is reached. The child receives a resolved optional-module path, a fixed `image`/`text` operation and a fixed byte limit; it does not receive clipboard text, user paths or executable code. stdout is binary and bounded, stderr is capped, no temporary image file is created, and the child is started with `shell: false`, the existing user permissions and hidden windows. The normal PowerShell-success path starts no child. The child is terminated on the 3-second native deadline or caller cancellation. The logical read settles only after the child emits `close`; until close, its active-read owner remains held and a second native read is rejected with a visible stopping/fault state. This keeps a late child result from being published or from clearing a newer clipboard operation.

The three cancellation times remain separate: UI cancellation is accepted immediately by the existing `AbortController`, the `readClipboardImage()` promise settles after the child termination is observed, and the native owner/read slot is released on child close. A native timeout or cancellation is fatal for this image gesture and does not fall through to text. Ordinary provider unavailability or a missing native module can still return null for the existing text/path fallback. PowerShell cancellation and image quota errors still stop before native fallback; a counted provider regression verifies that native is not called in those cases.

The controlled Windows regressions cover an empty PowerShell result followed by a native PNG fixture, production `readClipboardImage()` cancellation after `getImageBinary` begins, a child stuck synchronously in `hasImage`, post-close re-admission, and the PowerShell cancellation/quota boundary. The native fixture proves the isolated owner, transport, cancellation and cleanup path. It does not prove that every Windows clipboard producer uses PNG, CF_DIB or CF_DIBV5, and no claim of actual DIB/DIBV5 conversion or Win+Shift+S desktop acceptance is made here. Real Windows clipboard-format enumeration and native desktop key acceptance remain unexecuted.

The child adds one process start and bounded IPC only on the native fallback path. It may still allocate inside the native addon before termination; the transport limit is not a native peak-memory guarantee. Measurements for the controlled cancellation/timeout fixtures are recorded in `nativeClipboardCancellationRound` in `docs/performance/interactive-image-closeout-measurements.json`. Ordinary text input, render, mouse movement, PowerShell-success image reads and the explicit unsent-submission boundary do not enter this helper.

The current working-tree verification for this follow-up passes the complete `tests/github-review-regressions.test.ts` file (35/35) and `tests/image-placeholder-input.test.ts` (39/39), followed by `npm run check`, `npm run build:offline`, `npm run test:hot`, `npm test`, and `git diff --check`. These are local results for the controlled Windows transport fixtures; they do not replace real Win+Shift+S/Alt+V desktop input, clipboard-format enumeration, or online-service acceptance.

## Review follow-up after `ba8556b`

Windows text paste keeps the existing order: PowerShell STA helper, isolated native helper, then the already loaded native provider. An ordinary isolated-helper failure continues to the direct provider so ordinary text/path paste is preserved; cancellation and fatal bounded-read failures still stop the chain. The regression uses separate PowerShell, isolated-helper, and direct-provider seams and does not touch the user's clipboard. This direct provider is retained only as the existing compatibility fallback; the new isolated helper remains the bounded owner for the normal native path.

The changed candidate's affected full files pass 145/145 in the combined local run, with `npm run check`, `npm run build:offline`, `npm run test:hot`, `npm test`, and `git diff --check` also passing. These checks remain offline and do not constitute native Win+Shift+S/Alt+V desktop or online-service acceptance.

## Explorer image-file copy fallback (current working-tree fix)

Windows Explorer copies an image file as a `FileDrop`/CF_HDROP clipboard object. It may contain neither a bitmap nor ordinary Unicode text, so the image probe can correctly finish without an image and the old text helper then reported an empty clipboard. The Windows text helper now reads one Forms clipboard data object, preserves nonempty Unicode text, and otherwise serializes the bounded file list as quoted path-paste input. The existing path parser and attachment owner perform the extension, local-path, count, byte, and image validation; file names remain data and are never evaluated by PowerShell. This is clipboard file copy compatibility, not external file drag-and-drop support.

The production-chain regression executes the actual encoded PowerShell helper with an in-memory Forms `DataObject` source, then drives the real TUI Alt+V path for both the multimodal and auxiliary-vision offline configurations. It verifies Unicode and non-BMP names, multiple files, shell metacharacters, ordinary-text precedence, visible `未发送` state, zero pre-submit model calls, one post-submit route, preserved history associations, and unchanged source files. It does not overwrite the user's clipboard or claim native desktop/Explorer automation; native Win+Shift+S, actual clipboard format enumeration for a supplied image, and online services remain unverified.

When the same PowerShell clipboard path is used from WSL, quoted Windows FileDrop entries are converted through the existing WSL `wslpath -u` command before they reach the Linux image-path resolver. Each conversion has a one-second deadline, a bounded output, and the paste signal; an unavailable converter leaves the original text for the existing safe fallback. This conversion is only on the explicit clipboard-read path and is not used by ordinary input or native Windows execution.

## Native clipboard cancellation follow-up (`618c4d3`)

The non-Windows native clipboard branches now receive the active paste `AbortSignal`, including the synchronous `hasImage()` boundary and the asynchronous binary read. Cancellation is checked after the synchronous probe and after the native Promise settles, so a cancelled operation cannot publish an image or continue to xclip fallback. The underlying optional native API still has no guaranteed immediate cancellation operation; if it is already inside `getImageBinary()`, the logical owner remains occupied until that Promise settles. The new Linux/macOS fixture cases cover pre-cancel, cancellation after `hasImage()`, cancellation while the native Promise is pending, and native rejection, and verify that no secondary helper starts.

The Windows CI failure for `618c4d3` was a test-fixture issue exposed by the Windows runner's slower Forms startup: the controlled PowerShell fixture used the generic three-second command default and then fell through to the real native text fallback after timeout, producing a misleading `null.length` assertion. The production text reader now passes its explicit five-second deadline, and the fixture reports controlled helper failures as fatal so it cannot touch or silently read the real clipboard. A later Windows run showed that even five seconds is too close to the hosted runner's cold Forms startup for this in-memory integration fixture, so the fixture now uses a separate bounded 15-second process timeout while retaining an assertion that the production reader supplied five seconds. This changes test harness tolerance only; the production deadline, cancellation and fallback policy remain unchanged.

When queued submissions are restored into the interactive draft, attachment IDs are now checked against both existing draft records and earlier restored records. A duplicate, blank, or malformed restored ID receives a new host-owned ID while the immutable image content and ordering remain unchanged, so placeholder selection and removal cannot target another queued image. Canonical submission validation still rejects invalid IDs at the SDK boundary; this restore repair only handles legacy or independently queued metadata at the draft ownership boundary.
