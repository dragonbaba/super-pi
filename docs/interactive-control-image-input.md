...
The Windows CI failure for `7e375f0` was a test-fixture issue exposed by the Windows runner's slower Forms startup: the controlled PowerShell fixture used the generic three-second command default and then fell through to the real native text fallback after timeout, producing a misleading `null.length` assertion. The fixture now uses the same five-second Forms deadline as production and converts a controlled helper failure into a fatal test error, so it cannot touch or silently read the real clipboard. The production Windows behavior remains unchanged by this test hardening.
