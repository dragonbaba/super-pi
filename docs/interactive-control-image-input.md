...
- The persistent command bar is replaced by a short hint using the effective cursor/delete bindings. `/image`, `/image-remove` and `/image-clear` remain compatibility/explicit-file-entry paths; users do not need them for normal clipboard addition and deletion. Failed-submission recovery/cancellation remains independently reachable.
