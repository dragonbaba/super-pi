...
The clean confirmation list now shows bounded date/time, size, and a short session ID. The full exact path remains in the selected-file detail view, while an empty detail selection no longer reserves a large blank region. Main and Alt screen-model tests prepaint non-empty Unicode/color-like background rows, toggle the dialog 100 times, resize from 50×24 to 30×14, and verify background preservation, bounded frames, path visibility, and release on disposal. This is terminal-model evidence; no native desktop window was available in the offline run.

Node recovery now separates parse errors from `ERR_INVALID_ARG_TYPE`, module resolution, and ordinary runtime failures. It does not impose a length threshold or require a temporary file. A short script and a longer valid `node -e` containing Unicode, backslashes, and quotes each execute once in an isolated Bash fixture with no extra temporary-file or syntax-check subprocess. The nested-module syntax fixture preserves the first `SyntaxError` and `invalid.mjs` location; because the outer script already ran, the result is not labeled as an unexecuted policy refusal. A checked `.cjs`/`.mjs` file or stdin is guidance for quoting and reuse, not a mandatory security path.
