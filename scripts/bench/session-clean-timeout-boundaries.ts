...
interface SelectorRun {
	renderCalls: number; renderedLines: number; detailWrapCalls: number; maximumDetailLines: number;
	mainFrameWrites: number; altFrameWrites: number; totalFrameUtf16CodeUnits: number; maximumFrameUtf16CodeUnits: number;
	releasedFieldCount: number; ownerRefs: WeakRef<object>[]; detailRefs: WeakRef<object>[];
}
...
async function runSelectorFixture(iterations: number): Promise<SelectorRun> {
	...
	const ownerRefs = selectors.map(selector => new WeakRef<object>(selector));
	const detailRefs = selectors.map(selector => new WeakRef<object>((selector as unknown as { detailLines: string[] }).detailLines));
	...
	const detailLines = selectors.map(selector => (selector as unknown as { detailLines: string[] }).detailLines);
	const metrics = {
		...
		maximumDetailLines: Math.max(detailLines[0]!.length, detailLines[1]!.length),
	...
	const releasedFieldCount = selectors.reduce((count, selector) => {
		const state = selector as unknown as { items: unknown[]; detailLines: unknown[]; done?: unknown; getAvailableRows?: unknown };
		return count + (state.items.length === 0 ? 1 : 0) + (state.detailLines.length === 0 ? 1 : 0) + (state.done === undefined ? 1 : 0) + (state.getAvailableRows === undefined ? 1 : 0);
	}, 0);
	if (releasedFieldCount !== 8) throw new Error("selector retained owned records after async dispose");
	return { ...metrics, releasedFieldCount, ownerRefs, detailRefs };
}
...
interface SamplingSummary { sampledBytes: number; sampleCount: number; topSites: string }

async function samplePhase<T>(work: () => Promise<T>, siteLabel: string): Promise<{ value: T; sample: SamplingSummary }> {
	...
	let value: T;
	let profile: { head: SamplingNode };
	try { value = await work(); }
	finally {
		profile = (await inspector.post("HeapProfiler.stopSampling")).profile as unknown as { head: SamplingNode };
	...
	let sampledBytes = 0, sampleCount = 0, visited = 0;
	while (pending.length > 0 && visited++ < 10000) {
		const node = pending.pop()!;
		if (node.selfSize > 0) {
			sampledBytes += node.selfSize; sampleCount++;
	...
	return { value: value!, sample: { sampledBytes, sampleCount, topSites } };
}
...
let invocation = 0;
let finalSampling: { selector: SamplingSummary; shell: SamplingSummary; samplingInterval: number; gcExecuted: boolean } | undefined;
...
await runBenchmarkMain({
	...
	run: async () => {
		...
		if (sampleThisRun) globalThis.gc?.();
		const beforeHeap = sampleThisRun ? process.memoryUsage().heapUsed : 0;
		const started = performance.now();
		const selectorResult = sampleThisRun ? await samplePhase(() => runSelectorFixture(iterations), "selector") : { value: await runSelectorFixture(iterations), sample: { sampledBytes: 0, sampleCount: 0, topSites: "" } };
		const shellResult = sampleThisRun ? await samplePhase(async () => runShellFixture(iterations), "shell") : { value: runShellFixture(iterations), sample: { sampledBytes: 0, sampleCount: 0, topSites: "" } };
		const elapsedMs = performance.now() - started;
		const selectorWeakReleased = await releaseDiagnostics(selectorResult.value.ownerRefs);
		const detailWeakReleased = await releaseDiagnostics(selectorResult.value.detailRefs);
		const gcExecuted = sampleThisRun && typeof globalThis.gc === "function";
		const postGcHeapDeltaBytes = gcExecuted ? process.memoryUsage().heapUsed - beforeHeap : 0;
		if (sampleThisRun) finalSampling = { selector: selectorResult.sample, shell: shellResult.sample, samplingInterval: 8192, gcExecuted };
		return {
			selectorRenderCalls: selectorResult.value.renderCalls, selectorRenderedLines: selectorResult.value.renderedLines,
			selectorDetailWrapCalls: selectorResult.value.detailWrapCalls, selectorMaximumDetailLines: selectorResult.value.maximumDetailLines,
			...
			selectorReleasedFieldCount: selectorResult.value.releasedFieldCount, selectorWeakOwnerReleased: selectorWeakReleased, selectorWeakDetailReleased: detailWeakReleased,
			selectorSampledAllocationBytes: selectorResult.sample.sampledBytes, selectorAllocationSampleCount: selectorResult.sample.sampleCount,
			...
			shellSampledAllocationBytes: shellResult.sample.sampledBytes, shellAllocationSampleCount: shellResult.sample.sampleCount,
			postGcHeapDeltaBytes, fixtureElapsedMs: elapsedMs,
		};
	},
	observations: () => ({
		...
		sampling: sampleRequested ? "one final measured run; selector and shell phases sampled separately" : "disabled; timing has no profiler or forced-GC pass",
		...
		postGcHeapMeaning: "final-run heapUsed delta after optional exposed GC; not cumulative allocation, peak memory, sampled bytes, or leak bytes",
	}),
});
