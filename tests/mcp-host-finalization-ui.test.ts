...
for(const scenario of ["budget","prepare","success","mutation","replacement"])test(`MCP final UI disposition: ${scenario}`,async()=>{
  ...
  const original=scenario==="budget"||scenario==="prepare"?"ORIGINAL_SUCCESS ".repeat(6000):"ORIGINAL_SUCCESS";
  ...
  let ends=0, disposition:any;
  const session:any=fixture.session, mode=fixture.internal;
  const unsubscribe=fixture.session.subscribe((event:any)=>{if(event.type==="message_end"){ends++;disposition=event.toolResultMessageEndDisposition;}return mode.handleEvent(event);});
  try{
    ...
    if(scenario==="prepare")message.content=[{type:"text",text:42}];
    ...
    assert.equal(disposition,rejected?"host-finalized":scenario==="success"?"none":scenario==="replacement"?"replacement-returned":"handler-may-have-mutated");
    assert.equal(counts.canonicalPayloadRefreshes,scenario==="success"?0:1);
    ...
    assert.equal(counts.pendingEntries,0);assert.equal(counts.attachedEntries,0);
    ...
    assert.equal(owner.counters.artifactDescriptorsCreated,0);assert.equal(owner.counters.continuationCursorStringsCreated,0);
