...
test("128-entry insertion eviction, metadata-only admission and release", () => {
	...
	for (let i = 0; i < 129; i++) assert.equal(ledger.admit(record(String(i)), 512), true);
	...
	assert.equal(ledger.admit(input, 512), true);
...
test("metadata bound, oversized locations and argument inputs are rejected", () => {
	const ledger = new EvidenceLedger();
	assert.equal(ledger.admit({ ...record("huge"), location: "x".repeat(8193) }, 512), false);
	assert.equal(ledger.admit({ ...record("huge"), relativePath: "x".repeat(256 * 1024) }, 512), false);
	...
	for (let i = 0; i < 128; i++) ledger.admit({ ...record(String(i)), location: "x".repeat(8192), modelTokens: 16384 }, 16384);
	assert.ok(ledger.counters.entries > 0);
	assert.ok(ledger.counters.entries < 128);
	assert.ok(ledger.counters.metadataBytes <= EVIDENCE_MAX_METADATA_BYTES);
	ledger.dispose();
	assert.equal(ledger.admit(record("after"), 512), false);
	assert.equal(ledger.counters.metadataBytes, 0);
});
...
test("mutation/branch generations invalidate completed evidence before work", () => {
	const ledger = new EvidenceLedger();
	ledger.admit(record("old"), 512);
	ledger.mutate();
	assert.equal(ledger.lookup("old"), undefined);
	assert.equal(ledger.admit(record("late-completion"), 512), false);
	ledger.changeBranch();
