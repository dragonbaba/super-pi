...
Stateful agent with tool execution and event streaming. Built on `@super-pi/ai`.

## Installation

```bash
npm install @super-pi/agent-core
```
...
```typescript
import { Agent } from "@super-pi/agent-core";
...
```typescript
declare module "@super-pi/agent-core" {
...
```typescript
import { Agent, streamProxy } from "@super-pi/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@super-pi/agent-core";
