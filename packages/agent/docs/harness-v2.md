...
Implementation packages derive focused checks from this design. Historical QA matrices are retained in the permanent migration archive rather than the source distribution. Old tests are evidence, not specification: a case is restored only when it still expresses a target-design invariant and current coverage does not already exist.
