...
```ts
import { PiClient, type ByteTransportFactory } from "@super-pi/client";
...
```ts
import { PiClient } from "@super-pi/client";
import { createUnixTransportFactory } from "@super-pi/client/unix";
...
The `@super-pi/client` root remains transport- and runtime-neutral. Importing the Node-compatible transport requires the explicit `@super-pi/client/unix` subpath.
