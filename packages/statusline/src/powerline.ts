import type { Theme } from "@super-pi/coding-agent";
...
export function renderPowerlineStatuslineLines(
	width: number,
	items: RenderItem[],
	...
	for (const item of items) {
		if (item.name !== LINE_BREAK_SEGMENT_NAME) {
			segments.push(item);
			continue;
		}
		renderedLines.push(fitPowerlineSegments(segments, width, config));
		segments = [];
	}
	renderedLines.push(fitPowerlineSegments(segments, width, config));
...
export function renderPowerlineStatusline(
	...
	for (const item of items) {
		...
		if (hasRenderedLine) output += "\n";
		output += fitPowerlineSegments(segments, width, config);
		hasRenderedLine = true;
		segments = [];
	}
	if (hasRenderedLine) output += "\n";
	return output + fitPowerlineSegments(segments, width, config);
}
...
function formatBlockText(
	...
	const texts: string[] = [];
	for (const segment of block.segments) texts.push(formatSegmentText(segment));
