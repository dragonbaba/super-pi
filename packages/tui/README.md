...
```typescript
import { type TUI, Text, Editor, ProcessTerminal, TuiMainScreen, matchesKey } from "@super-pi/tui";
...
```typescript
import { type TUI, TuiAltScreen, TuiMainScreen } from "@super-pi/tui";
...
`Terminal.write()` is for terminal controls; observe or await any returned Promise. Renderers use the callback-driven frame lane: register one stable `setFrameWriteCompletionListener()`, then call `writeFrame(data, generation)`. Completion requires a successful Writable callback and, after a false return, `drain`. Normal frame writes have no Promise or `AbortController` and no absolute timeout. `TuiBase` combines content and cursor output from one render into one atomic frame. A lifecycle deadline may logically cancel the current queue cycle, but it does not permanently poison a resumable TUI: the next `start()` opens a new logical cycle. An OS write is not cancellable, so the orphan keeps exclusive physical writer ownership until its own callback and drain settle; one replacement may wait in fixed terminal slots without consuming stale events or being counted as a physical write. Real stream callback/error/close failures remain permanent. Use `stop()` for a resumable mode switch, suspend, or external-editor boundary; use `dispose()` for final ownership release.
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@super-pi/tui";
...
The cursor remains hidden by default. This keeps the fake cursor rendering, while still positioning the hardware cursor for terminals that track IME candidate windows with hidden cursors. Some terminals require a visible hardware cursor for IME positioning; enable it with the renderer constructor's `showHardwareCursor` argument, `setShowHardwareCursor(true)`, or `SP_HARDWARE_CURSOR=1`. The `Editor` and `Input` built-in components already implement this interface.

**Container components with embedded inputs:** When a container component (dialog, selector, etc.) contains an `Input` or `Editor` child, the container must implement `Focusable` and propagate the focus state to the child:

```typescript
import { Container, type Focusable, Input } from "@super-pi/tui";
...
```typescript
import { CombinedAutocompleteProvider } from "@super-pi/tui";
...
```typescript
import { matchesKey, Key } from "@super-pi/tui";
...
```typescript
import { visibleWidth, truncateToWidth, wrapTextWithAnsi } from "@super-pi/tui";
...
```typescript
import { matchesKey, Key, truncateToWidth } from "@super-pi/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@super-pi/tui";
import type { Component } from "@super-pi/tui";
...
Set `SP_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
SP_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx test/chat-simple.ts
```
