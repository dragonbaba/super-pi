...
function restorePlanModeInstructionState(ctx: ExtensionContext): PlanModeInstructionState | undefined {
	const branch = ctx.sessionManager.getBranch();
	for (let index = branch.length - 1; index >= 0; index--) {
		const entry = branch[index];
		if (entry.type !== "custom_message") continue;
		if (entry.customType !== PLAN_MODE_INSTRUCTION_MESSAGE_TYPE) continue;
		const details = entry.details as { version?: unknown; state?: unknown } | undefined;
		if (details?.version !== 1) continue;
		if (details.state === "active" || details.state === "inactive") return details.state;
	}
	return undefined;
}

function planModeInstructionMessage(state: PlanModeInstructionState) {
	return {
		customType: PLAN_MODE_INSTRUCTION_MESSAGE_TYPE,
		content:
			state === "active"
				? `The following Plan Mode instructions are now active and supersede any earlier Plan Mode state:\n\n${buildPlanModePrompt()}`
				: "Plan Mode is now inactive. Earlier Plan Mode instructions no longer apply; follow the ordinary session instructions and currently available tools.",
		display: false,
		details: { version: 1, state },
	};
}
...
export default function planMode(pi: ExtensionAPI, dependencies: PlanModeDependencies = {}) {
	...
	let instructionState: PlanModeInstructionState | undefined;
	...
	// Plan-only schemas should not be active before lifecycle restoration.
	...
	pi.on("session_start", async (event, ctx) => {
		...
		restoreState(ctx);
		instructionState = restorePlanModeInstructionState(ctx);
	...
	pi.on("before_agent_start", (event, ctx) => {
		...
		const nextInstructionState: PlanModeInstructionState = state.enabled ? "active" : "inactive";
		const instructionChanged = instructionState !== nextInstructionState;
		if (!state.enabled) {
			if (!instructionChanged || instructionState === undefined) return;
			instructionState = nextInstructionState;
			return { message: planModeInstructionMessage(nextInstructionState) };
		}
