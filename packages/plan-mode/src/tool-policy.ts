import type { ToolInfo } from "@super-pi/coding-agent";
...
const READ_ONLY_COMMANDS = new Set([
	...
	"whereis",
	"type",
	// Environment enumeration is not read-only.
	"uname",
	"whoami",
	"id",
	"date",
...
export function classifyPlanModeTool(tool: ToolInfo): PlanModeToolPolicy {
  if (tool.name === "delete" || tool.name === "move") return "blocked";
...
function isSafeStructuredCommand(
	...
	if (command === "npm") {
		...
		return ["list", "ls", "view", "info", "search", "outdated", "audit"].includes(
			subcommand ?? "",
		);
	}
	return false;
