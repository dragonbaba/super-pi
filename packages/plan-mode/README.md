...
This source is bundled by Super Pi's repository configuration.

```bash
superpi install ./packages/plan-mode
```

Try without installing permanently:

```bash
superpi -e ./packages/plan-mode
```
...
Open **Settings** from an inactive `/plan` menu to edit one flat group of four workflow choices: **Plan thinking**, **Plan tools**, **After Implement**, and **Export destination**. You can also edit `$SP_CODING_AGENT_DIR/config/sp-plan-mode.json` (normally `~/.sp/agent/config/sp-plan-mode.json`) manually; `safeSubcommands` remains JSON-only. The file is optional, is read at session start, and is created only after an explicit Settings save or manual edit.
