...
```bash
npm install @super-pi/telemetry
```
...
```typescript
import { NOOP_TELEMETRY_CONTEXT } from '@super-pi/telemetry';
...
```typescript
import { InMemoryTelemetryContext } from '@super-pi/telemetry';
...
```typescript
import { AGENT_TELEMETRY_SCHEMAS } from '@super-pi/agent-core';
...
- `@super-pi/telemetry` owns the vendor-neutral contract, no-op and in-memory reference contexts, schema utilities, and adapter conformance suite;
- `@super-pi/ai` accepts and propagates `telemetryContext` in provider request options but owns no telemetry schema;
- `@super-pi/agent-core` owns and exports the pi AI-request and harness schemas, their combined readonly schema tuple, and typed span helpers.
