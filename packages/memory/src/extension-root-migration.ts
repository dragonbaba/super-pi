...
async function moveFileSafe(source: string, target: string): Promise<void> {
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      if (code === "EXDEV") break;
      const retryableWindowsLock = process.platform === "win32" && (code === "EBUSY" || code === "EPERM");
      if (!retryableWindowsLock || attempt >= WINDOWS_RENAME_RETRY_ATTEMPTS) throw error;
...
async function acquireMigrationLease(legacyRoot: string, targetRoot: string): Promise<AtomicLockLease> {
  // Migration roots may be one-shot (especially tests and renamed legacy roots),
  // so do not retain their coordinator in the process-wide shared map.
    ...
    if (lease) {
      ...
      return {
        ...
        release: () => {
          if (released) return;
          released = true;
          try { lease.release(); } finally { coordinator.close(); }
        },
      };
    }
...
async function migrateDatabaseGeneration(
  ...
  try {
    ...
    const releaseWriteLockForRetirement = () => {
      if (!writeLock) return;
    ...
    if (sourceState.isSymbolicLink()) {
      throw new Error("sessions.db symbolic links are not migrated; replace the link with a verified regular SQLite file and retry");
      ...
      throw new Error("sessions.db is not a regular file");
    }

    if (!corruptGeneration) {
      // Windows does not permit renaming SQLite generation files while this
      // process still owns an open handle. The snapshot is complete and the
      // immediate transaction has excluded cooperating writers up to here, so
      // release our own handle before retiring the source generation. A truly
      // external live handle still makes rename fail closed.
      releaseWriteLockForRetirement();
      // Closing the final WAL connection can checkpoint and remove -wal/-shm,
      // so refresh the generation immediately before retirement.
      generationNames = await databaseFilesAt(legacyRoot);
      try {
        retired = await moveDatabaseGeneration(generationNames, legacyRoot, retirementDir, retire);
      } catch (error) {
