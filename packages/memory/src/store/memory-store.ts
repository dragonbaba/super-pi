...
const RECOVERY_PRUNE_INTERVAL_MS = 60 * 60 * 1000;
const ACTIVE_RECOVERY_MAX_COUNT = 8;
const ACTIVE_RECOVERY_RETAIN_COUNT = ACTIVE_RECOVERY_MAX_COUNT - 1;
const ACTIVE_RECOVERY_MAX_BYTES = 8 * 1024 * 1024;
const RETIRED_RECOVERY_MAX_AGE_MS = 30 * 24 * 60 * 60 * 1000;
const RETIRED_RECOVERY_MAX_COUNT = 8;
const RETIRED_RECOVERY_MAX_BYTES = 8 * 1024 * 1024;
...
export class MemoryStore {
  ...
  private recoveryPrunedAt = new Map<string, number>();
  ...
  private get memoryDir(): string {
    return this.config.memoryDir ?? path.join(AGENT_ROOT, "@super-pi/memory");
  }
  ...
  private async _add(
    ...
    const newTotal = appendedCharCount(entries, encoded);
    if (newTotal > limit) {
  ...
  private async fifoEvictAndAdd(
    ...
    let total = appendedCharCount(entries, encoded);
    while (total > limit && firstRetainedIndex < entries.length) {
      const evicted = entries[firstRetainedIndex++]!;
      total -= evicted.length + ENTRY_DELIMITER.length;
      evictedEntries.push(this.stripMetadata(evicted));
    }

    const remaining = entries.slice(firstRetainedIndex);
  ...
  evaluateMemoryReplacement(nextEntries: string[]):
    ...
    for (const entry of normalized) {
      const scanError = scanContent(entry);
      if (scanError) return { success: false, error: scanError };
    }

    const today = new Date().toISOString().split("T")[0];
    const existingByText = new Map(this.memoryEntries.map((entry) => [this.stripMetadata(entry), entry]));
    const encoded = normalized.map((entry) => existingByText.get(entry) ?? this.encodeEntry(entry, today, today));
    const chars = encoded.length ? encoded.join(ENTRY_DELIMITER).length : 0;
    const limit = this.charLimit("memory");
    if (chars > limit) {
      return { success: false, error: `Organization proposal would put memory at ${chars}/${limit} chars.`, chars, limit };
    }
    return { success: true, chars, limit };
  }

  /**
   * Replace the complete MEMORY.md entry set in one locked publication, but
   * only while the visible source snapshot still matches what the caller
   * reviewed. Used by confirmation-gated organization so a stale proposal can
   * never overwrite a concurrent memory update.
   */
  async replaceMemoryEntriesIfUnchanged(
    expectedEntries: string[],
    nextEntries: string[],
  ): Promise<MemoryResult> {
    return this.runTargetMutation("memory", async () => {
      await this.syncTargetFromDiskIfChanged("memory");
      const currentVisible = this.memoryEntries.map((entry) => this.stripMetadata(entry));
      if (currentVisible.length !== expectedEntries.length
        || currentVisible.some((entry, index) => entry !== expectedEntries[index])) {
        return {
          success: false,
          error: "Memory changed after the organization preview. No changes were applied; generate a new preview.",
        };
      }

      const evaluation = this.evaluateMemoryReplacement(nextEntries);
      ...
      const existingByText = new Map(this.memoryEntries.map((entry) => [this.stripMetadata(entry), entry]));
      const encoded = normalized.map((entry) => existingByText.get(entry) ?? this.encodeEntry(entry, today, today));
      this.memoryEntries = encoded;
      await this.saveToDisk("memory");
      return this.successResponse("memory", "Confirmed memory organization applied atomically.");
    });
  }
  ...
  private decodeEntry(raw: string): { text: string; created: string; lastReferenced: string; project: string | null } {
    const match = raw.match(ENTRY_METADATA_PATTERN);
    if (match) {
  ...
  /**
   * Startup-only recovery maintenance for a target that may no longer receive
   * writes. The canonical mutation lock keeps this sweep from racing a live
   * MemoryStore writer; force bypasses only the per-instance hourly throttle.
   */
  async pruneRecoveryArtifactsForFile(filePath: string): Promise<void> {
    await withMarkdownMutationLock(filePath, () => this.pruneRecoveryFiles(filePath, true));
  }

  private async pruneRecoveryFiles(filePath: string, force = false): Promise<void> {
    const now = Date.now();
    const directory = path.dirname(filePath);
    const baseName = path.basename(filePath);
    const recoveryPrefix = `.${baseName}.recovery-`;
    const retiredPrefix = `.${baseName}.retired-`;
    const conflictPrefix = `.${baseName}.conflict-local-`;
    const activeCutoff = now - RECOVERY_ACTIVE_GRACE_MS;
    try {
      const names = await fs.readdir(directory);
      // A cheap name-only pass runs on every save so cross-process writers
      // cannot exceed the count bounds. lstat/copy/sort work is throttled
      // while all artifact groups remain below their limits.
      const lastPrunedAt = this.recoveryPrunedAt.get(filePath) ?? 0;
      if (!force && now - lastPrunedAt < RECOVERY_PRUNE_INTERVAL_MS) {
        let activeCount = 0;
        let retiredCount = 0;
        let conflictCount = 0;
        for (const name of names) {
          if (isGeneratedArtifactName(name, recoveryPrefix)) activeCount++;
          else if (isGeneratedArtifactName(name, retiredPrefix)) retiredCount++;
          else if (isGeneratedArtifactName(name, conflictPrefix)) conflictCount++;
        }
        if (activeCount < ACTIVE_RECOVERY_MAX_COUNT
          && retiredCount <= RETIRED_RECOVERY_MAX_COUNT
          && conflictCount <= CONFLICT_MAX_COUNT) {
          return;
        }
      }
      if (!force) this.recoveryPrunedAt.set(filePath, now);
      const activeCandidates: Array<{ path: string; state: Stats }> = [];
      for (const name of names) {
        if (!isGeneratedArtifactName(name, recoveryPrefix)) continue;
        const recoveryPath = path.join(directory, name);
        try {
          const state = await fs.lstat(recoveryPath);
          if (state.isFile()) activeCandidates.push({ path: recoveryPath, state });
        } catch {
        }
      }
      activeCandidates.sort((left, right) => right.state.mtimeMs - left.state.mtimeMs);
      let activeCount = 0;
      let activeBytes = 0;
      for (const item of activeCandidates) {
        const withinGrace = item.state.mtimeMs >= activeCutoff;
        const withinCount = activeCount < ACTIVE_RECOVERY_RETAIN_COUNT;
        const withinBytes = activeBytes + item.state.size <= ACTIVE_RECOVERY_MAX_BYTES;
        if (withinGrace && withinCount && withinBytes) {
          activeCount++;
      ...
      const retiredNames = await fs.readdir(directory);
      const candidates: Array<{ path: string; state: Stats }> = [];
      for (const name of retiredNames) {
        if (!isGeneratedArtifactName(name, retiredPrefix)) continue;
        const retiredPath = path.join(directory, name);
        try {
          const state = await fs.lstat(retiredPath);
          if (state.isFile()) candidates.push({ path: retiredPath, state });
        } catch {
        }
      }
      const maxAgeCutoff = now - RETIRED_RECOVERY_MAX_AGE_MS;
      candidates.sort((left, right) => right.state.mtimeMs - left.state.mtimeMs);
      let retainedCount = 0;
      let retainedBytes = 0;
      ...
      const conflictNames = await fs.readdir(directory);
      const conflictCandidates: Array<{ path: string; state: Stats }> = [];
      for (const name of conflictNames) {
        if (!isGeneratedArtifactName(name, conflictPrefix)) continue;
        const conflictPath = path.join(directory, name);
        try {
          const state = await fs.lstat(conflictPath);
          if (state.isFile()) conflictCandidates.push({ path: conflictPath, state });
        } catch {
        }
      }
      const graceCutoff = now - CONFLICT_ACTIVE_GRACE_MS;
      const conflictMaxAgeCutoff = now - CONFLICT_MAX_AGE_MS;
      conflictCandidates.sort((left, right) => right.state.mtimeMs - left.state.mtimeMs);
      let conflictCount = 0;
      let conflictBytes = 0;
...
function appendedCharCount(entries: readonly string[], appended: string): number {
  let total = appended.length;
  for (const entry of entries) total += entry.length + ENTRY_DELIMITER.length;
  return total;
}
