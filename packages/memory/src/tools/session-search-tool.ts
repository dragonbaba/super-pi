...
function registerLegacySessionSearchTool(pi: ExtensionAPI, dbManager: DatabaseManager): void {
  pi.registerTool({
    name: 'session_search',
    label: 'Session Search',
    description: 'Search prior Pi conversations by query and optional project/role filters. Returns bounded dated snippets; large messages report their original length.',
    parameters: Type.Object({
    ...
    execute: async (_id: string, args: { query: string; project?: string; role?: string; limit?: number; snippetChars?: number }) => {
      ...
      if (totalMessages === 0) {
        const result: SearchResult = { success: false, message: 'No indexed sessions are available. The legacy search variant only queries an existing index; configure the anchors variant to search session JSONL files directly.' };
        return { content: [{ type: 'text' as const, text: result.message! }], details: result };
      }
