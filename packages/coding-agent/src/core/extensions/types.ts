...
export interface ExtensionUIContext {
	 ...
	 * extend `CustomEditor` from `@super-pi/coding-agent` and call
	 * `super.handleInput(data)` for keys you don't handle.
	 *
	 * @example
	 * ```ts
	 * import { CustomEditor } from "@super-pi/coding-agent";
	 *
...
export interface ContextUsage {
	...
	percent: number | null;
	/** Whether the estimate comes from provider usage or materialized session messages. */
	source?: "provider" | "materialized";
	/** False when no transport-verifiable post-compaction usage epoch is available. */
	epochVerified?: boolean;
	/** Automatic threshold compaction is blocked because paid work has no committed boundary. */
	automaticCompactionBlocked?: "paid-boundary-uncommitted";
}
...
export interface SessionBeforeCompactEvent {
	...
	/** Immutable auth snapshot resolved once by Core for this compaction attempt. */
	auth?: ProviderRequestAuthSnapshot;
	customInstructions?: string;
...
export interface ExtensionAPI {
	...
	/** Build the active provider request payload without network I/O. */
	buildProviderRequestPayload(input: ProviderRequestPayloadInput): Promise<Record<string, unknown> | undefined>;
	/** Execute fail-closed provider-native compaction through the active provider runtime. */
	compactProviderRequestPayload(
...
export interface ProviderRequestPayloadInput {
	systemPrompt: string;
	messages: AgentMessage[];
}

export type BuildProviderRequestPayloadHandler = (
...
export type CompactProviderRequestPayloadHandler = (
	input: ProviderRequestCompactionInput,
) => Promise<ProviderRequestCompactionResult | undefined>;
...
export interface ExtensionActions {
	...
	buildProviderRequestPayload: BuildProviderRequestPayloadHandler;
	compactProviderRequestPayload: CompactProviderRequestPayloadHandler;
