...
import { type ImageContent, type Message, type TextContent, type Usage, uuidv7 } from "@super-pi/ai";
...
export interface CompactionEntry<T = unknown> extends SessionEntryBase {
	...
	tokensBefore: number;
	/** Self-contained context tail. When present, it replaces referenced pre-compaction entries. */
 ...
 * Encodes cwd into a safe directory name under ~/.super-pi/agent/sessions/.
 */
...
export class SessionManager {
		...
		if (existsSync(this.sessionFile)) {
			...
			if (this.fileEntries.length === 0) {
				...
				if (statSync(explicitPath).size > 0) {
					throw new Error(`Session file is not a valid Super Pi session: ${explicitPath}`);
				}
				this.newSession();
	 ...
	 * @param sessionDir Optional session directory. If omitted, uses default (~/.super-pi/agent/sessions/<encoded-cwd>/).
	 */
