...
export function resolveCliModel(options: {
	...
	let resolution = parseModelPattern(pattern, candidates, {
		allowInvalidThinkingLevelFallback: false,
	});
	if (!provider && resolution.model) {
		let fuzzyPattern = pattern;
		if (resolution.thinkingLevel) fuzzyPattern = pattern.substring(0, pattern.lastIndexOf(":"));
		if (!findExactModelReferenceMatch(fuzzyPattern, candidates)) {
			const normalizedPattern = fuzzyPattern.toLowerCase();
			const fuzzyMatches: Model<any>[] = [];
			...
			if (matchingProviders.length > 1) {
				...
				if (authenticatedProviders.length === 1) {
					const authenticatedCandidates = candidates.filter(
						(candidate) => candidate.provider === authenticatedProviders[0],
					);
					resolution = parseModelPattern(pattern, authenticatedCandidates, {
						allowInvalidThinkingLevelFallback: false,
					});
				} else {
					matchingReferences.sort();
					const authHint =
					...
					return {
						...
						warning: undefined,
						thinkingLevel: undefined,
						error: `Model "${cliModel}" is ambiguous across providers: ${matchingReferences.join(", ")}. ${authHint} Use --provider or provider/model.`,
					};
				}
			}
		}
	}
	const { model, thinkingLevel, warning } = resolution;
