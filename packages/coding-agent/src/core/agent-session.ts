...
import {
	...
	type ProviderRequestCompactionInput,
...
function getBuiltinExecutionPath(name: string, cwd: string) {
	switch (name) {
		case "read": return { access: "read" as const, cwd, argument: "path" };
		case "grep":
		case "find":
		case "ls": return { access: "read" as const, cwd, argument: "path", defaultPath: "." };
		case "edit":
		case "write": return { access: "write" as const, cwd, argument: "path" };
...
export interface AgentSessionConfig {
	...
	/** Provider-owned request serializer used for extension request previews. */
	providerRequestPayloadBuilder?: (input: {
		model: Model<any>;
		systemPrompt: string;
		messages: AgentMessage[];
		tools: AgentTool[];
	...
	/** Provider-owned native compactor. */
	...
	}) => Promise<ProviderRequestCompactionResult | undefined>;
...
function hasPendingPaidCompactionBoundary(entries: SessionEntry[], model: Model<any> | undefined): boolean {
	...
	for (let index = entries.length - 1; index > latestCompactionIndex; index--) {
		const entry = entries[index];
		if (entry.type !== "custom" || entry.customType !== "compaction-usage-ledger-v2") continue;
		const data = entry.data;
		if (
			data && typeof data === "object" && !Array.isArray(data) &&
			(data as Record<string, unknown>).schemaVersion === 2 &&
			(data as Record<string, unknown>).component === "remote_compaction" &&
			(data as Record<string, unknown>).status === "provider_completed" &&
			(data as Record<string, unknown>).model === modelName &&
			typeof (data as Record<string, unknown>).operationId === "string"
		) return true;
	}
	return false;
}

function getPersistedMaterializedBaseline(latestCompaction: CompactionEntry): number | null {
	const details = latestCompaction.details as
		| { contextAccounting?: { schemaVersion?: number; materializedTokensAfter?: number } }
		| undefined;
	const accounting = details?.contextAccounting;
	if (accounting?.schemaVersion === 1 && Number.isFinite(accounting.materializedTokensAfter)) {
		return accounting.materializedTokensAfter!;
	}
	if (!Array.isArray(latestCompaction.retainedTail)) return null;
...
interface CompactionAwareEstimate {
	tokens: number;
	usageTokens: number;
	trailingTokens: number;
	lastUsageIndex: number | null;
	source: "provider" | "materialized";
	epochVerified: boolean;
	materializedBaselineTokens: number | null;
	paidBoundaryPending: boolean;
}

function estimateCompactionAwareContextTokens(
	entries: SessionEntry[], messages: AgentMessage[], model: Model<any> | undefined,
): CompactionAwareEstimate {
	const paidBoundaryPending = hasPendingPaidCompactionBoundary(entries, model);
	let latestCompactionIndex = -1;
	for (let index = entries.length - 1; index >= 0; index--) {
		if (entries[index].type === "compaction") {
			latestCompactionIndex = index;
	...
	if (latestCompactionIndex < 0) {
		return {
			...estimateContextTokens(messages), source: "provider", epochVerified: true,
			materializedBaselineTokens: null, paidBoundaryPending,
		};
	}
	const estimated = estimateMessagesTokens(messages);
	return {
		tokens: estimated, usageTokens: 0, trailingTokens: estimated, lastUsageIndex: null,
		source: "materialized", epochVerified: false,
		materializedBaselineTokens: getPersistedMaterializedBaseline(entries[latestCompactionIndex] as CompactionEntry),
		paidBoundaryPending,
	};
}

function shouldAutoThresholdCompact(
	estimate: CompactionAwareEstimate,
	contextWindow: number,
	settings: ReturnType<SettingsManager["getCompactionSettings"]>,
): boolean {
	if (estimate.paidBoundaryPending || !shouldCompact(estimate.tokens, contextWindow, settings)) return false;
	if (estimate.source === "provider") return true;
	const threshold = contextWindow - settings.reserveTokens;
	return Number.isFinite(estimate.materializedBaselineTokens) && estimate.materializedBaselineTokens! <= threshold;
}

function withContextAccounting(
	details: unknown,
	model: Model<any>,
	reason: "manual" | "threshold" | "overflow",
	summary: string,
	tokensBefore: number,
	retainedTail: AgentMessage[] | undefined,
	contextWindow: number,
	settings: ReturnType<SettingsManager["getCompactionSettings"]>,
): Record<string, unknown> {
	const materializedTokensAfter = Array.isArray(retainedTail)
		? estimateMessagesTokens([
				{ role: "compactionSummary", summary, tokensBefore, timestamp: Date.now() },
				...retainedTail,
			])
		: null;
	const baseDetails = details && typeof details === "object" && !Array.isArray(details) ? details : {};
	return {
		...baseDetails,
		contextAccounting: {
			schemaVersion: 1,
			owner: { provider: model.provider, model: model.id },
			providerUsageState: "unverified",
			estimateSource: "materialized",
			materializedTokensAfter,
			contextWindow,
...
export class AgentSession {
		...
		this.agent.prepareNextTurnWithContext = async (turn, signal) => {
			...
			if (turn.toolResults.length > 0 && !signal?.aborted && !this.isCompacting) {
				...
				if (settings.enabled && contextWindow > 0) {
					const estimate = estimateCompactionAwareContextTokens(
						this.sessionManager.getBranch(), turn.context.messages, this.model,
					);
					if (shouldAutoThresholdCompact(estimate, contextWindow, settings)) {
						const beforeCompactionId = getLatestCompactionEntry(this.sessionManager.getBranch())?.id;
						await this._runAutoCompaction("threshold", false, estimate.tokens);
	...
	async buildProviderRequestPayload(
		input: ProviderRequestPayloadInput,
	): Promise<Record<string, unknown> | undefined> {
		const model = this.model;
		const builder = this._providerRequestPayloadBuilder;
		if (!model || !builder) return undefined;
	...
	async compactProviderRequestPayload(
		...
		if (!auth.apiKey) throw new Error(`Provider-native compaction requires an API key for ${model.provider}.`);
		return compactor({
			model: auth.model,
			apiKey: auth.apiKey,
			...
			sessionId: this.sessionManager.getSessionId(),
			regularPayload: input.regularPayload,
			signal: input.signal,
			shapeDiagnostics: input.shapeDiagnostics,
		});
	}
	...
	async compact(customInstructions?: string): Promise<CompactionResult> {
		...
		try {
			...
			preparation.tokensBefore = estimateCompactionAwareContextTokens(
				pathEntries, this.agent.state.messages, this.model,
			).tokens;
			...
			details = withContextAccounting(
				details, this.model, "manual", summary, tokensBefore, retainedTail, this.model.contextWindow, settings,
			);
			...
			const savedCompactionEntry = newEntries.find((e) => e.type === "compaction" && e.id === savedCompactionId) as
				| CompactionEntry
				| undefined;
	...
	private async _checkCompaction(assistantMessage: AssistantMessage, skipAbortedCheck = true): Promise<boolean> {
		...
		const estimate = estimateCompactionAwareContextTokens(
			this.sessionManager.getBranch(), this.agent.state.messages, this.model,
		);
		if (shouldAutoThresholdCompact(estimate, contextWindow, settings)) {
			return await this._runAutoCompaction("threshold", false, estimate.tokens);
		}
		return false;
	}
	...
	private async _runAutoCompaction(
		...
		try {
			...
			if (providedCompaction) {
				summary = providedCompaction.summary;
				firstKeptEntryId = providedCompaction.firstKeptEntryId;
				tokensBefore = providedCompaction.tokensBefore;
				usage = providedCompaction.usage;
				details = providedCompaction.details;
				retainedTail = providedCompaction.retainedTail;
			} else {
				// Generate compaction result
				...
				const prefix =
					reason === "overflow"
						? stage === "authentication"
							? "Context overflow recovery authentication failed"
		...
		runner.bindCore(
			{
				...
				buildProviderRequestPayload: (input) => this.buildProviderRequestPayload(input),
				compactProviderRequestPayload: (input) => this.compactProviderRequestPayload(input),
	...
	getSessionStats(): SessionStats {
		...
		const entries = this.sessionManager.getEntries();
		for (const usage of getUnboundCompactionLedgerUsages(entries)) addUsageToTotals(usageTotals, usage);
	...
	getContextUsage(): ContextUsage | undefined {
		...
		const estimate = estimateCompactionAwareContextTokens(branchEntries, this.messages, model);
		...
		return {
			...
			epochVerified: estimate.epochVerified,
			automaticCompactionBlocked: estimate.paidBoundaryPending ? "paid-boundary-uncommitted" : undefined,
		};
	}
