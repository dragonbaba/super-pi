...
function executeCommand(commandConfig: string): string | undefined {
	if (commandResultCache.has(commandConfig)) {
		...
		commandResultCache.set(commandConfig, cached);
		return cached;
	}

	const result = executeCommandUncached(commandConfig);
	if (commandResultCache.size >= COMMAND_RESULT_CACHE_MAX_ENTRIES) {
		const oldest = commandResultCache.keys().next();
		if (!oldest.done) commandResultCache.delete(oldest.value);
	}
