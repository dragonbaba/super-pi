...
export class ModelRuntime implements Models {
	...
	static async create(options: CreateModelRuntimeOptions = {}): Promise<ModelRuntime> {
		...
		const runtime = new ModelRuntime(
			...
			modelsStore,
			providers,
			process.env.SP_OFFLINE === undefined,
		);
	...
	async refresh(options: ModelsRefreshOptions = {}): Promise<ModelsRefreshResult> {
		...
		} else if (!refreshOptions.allowNetwork) {
			// A global network pass refreshes catalogs above and updateModelSnapshot()
			// projects them through the existing configured-provider snapshot. Re-running
			// every auth/availability check here couples unrelated credential latency to
			// catalog refresh and can consume the caller's aggregate timeout.
			try {
				await this.queueAvailabilityRefresh(options.signal);
			} catch {
