import { realpath, stat } from "node:fs/promises";
import { realpathSync, statSync } from "node:fs";
import { resolve } from "node:path";

// Private in-process handoff across source/dist extension loaders; never serialized.
const CWD_BINDING = Symbol.for("super-pi.shell-cwd-binding");
// Weak backend identity registry, not an authorization/path cache. No history or scans.
const localBackends = new WeakMap<object, Function>();
export function registerLocalShellBackend<T extends { exec: Function }>(backend: T): T { localBackends.set(backend, backend.exec); return backend; }
export function isLocalShellBackend(backend: { exec: Function }): boolean { return localBackends.get(backend) === backend.exec; }

function assertDirectoryIdentity(requested: string, canonical: string, device: bigint, inode: bigint): void {
  let current: string, directory;
  try { current = realpathSync.native(requested); directory = statSync(current, { bigint: true }); }
  catch (cause) { throw new Error("[SHELL_CWD_CHANGED] Directory lookup failed after authorization; request fresh approval.", { cause }); }
  if (current !== canonical || !directory.isDirectory() || directory.dev !== device || directory.ino !== inode) {
    throw new Error("[SHELL_CWD_CHANGED] Directory identity changed after authorization; request fresh approval.");
  }
}

/** Invocation-owned directory facts. No cache, handle, timer or model-visible token. */
export class ShellCwdBinding {
  #authority?: () => void;
  #released = false;
  readonly requested: string;
  readonly canonical: string;
  readonly sessionRequested: string;
  readonly sessionCanonical: string;
  readonly #sessionDevice: bigint;
  readonly #sessionInode: bigint;
  readonly #device: bigint;
  readonly #inode: bigint;
  constructor(requested: string, canonical: string, device: bigint, inode: bigint,
    sessionRequested: string, sessionCanonical: string, sessionDevice: bigint, sessionInode: bigint) {
    this.requested = requested; this.canonical = canonical; this.#device = device; this.#inode = inode;
    this.sessionRequested = sessionRequested; this.sessionCanonical = sessionCanonical; this.#sessionDevice = sessionDevice; this.#sessionInode = sessionInode; Object.freeze(this);
  }
  get isReleased(): boolean { return this.#released; }
  setAuthority(authority: () => void): void { this.#authority = authority; }
  readonly beforeSpawn = (cwd: string): void => {
    if (this.#released || cwd !== this.canonical) throw new Error("[SHELL_CWD_CHANGED] Execution directory changed.");
    this.#authority?.();
    // Both the execution target and project-trust root were pinned before approval.
    this.assertSessionRoot();
    assertDirectoryIdentity(this.requested, this.canonical, this.#device, this.#inode);
  };
  matchesSessionRoot(canonical: string, device: number, inode: number): boolean {
    return this.sessionCanonical === canonical && Number(this.#sessionDevice) === device && Number(this.#sessionInode) === inode;
  }
  assertSessionRoot(): void {
    if (this.#released) throw new Error("[SHELL_CWD_CHANGED] Directory authority was released.");
    assertDirectoryIdentity(this.sessionRequested, this.sessionCanonical, this.#sessionDevice, this.#sessionInode);
  }
  release(): void { this.#released = true; this.#authority = undefined; }
}

export function getShellCwdBinding(input: unknown): ShellCwdBinding | undefined {
  return input && typeof input === "object" ? (input as Record<symbol, ShellCwdBinding>)[CWD_BINDING] : undefined;
}
export function attachShellCwdBinding(input: object, binding: ShellCwdBinding): void {
  Object.defineProperty(input, CWD_BINDING, { value: binding, configurable: true });
}
export async function prepareShellCwd(input: { cwd?: unknown }, sessionCwd: string): Promise<ShellCwdBinding | undefined> {
  const existing = getShellCwdBinding(input);
  if (input.cwd === undefined) {
    if (existing) {
      if (!existing.isReleased) throw new Error("[SHELL_CWD_CHANGED] Bound cwd was removed before execution.");
      Object.defineProperty(input, CWD_BINDING, { value: undefined, configurable: true });
    }
    return undefined;
  }
  if (typeof input.cwd !== "string" || input.cwd.length === 0 || input.cwd.length > 4096 || input.cwd.includes("\0")) throw new Error("[SHELL_CWD_INVALID] cwd must be a nonempty literal directory path.");
  // No shell/home/MSYS expansion: filesystem paths are interpreted by the local backend.
  const sessionRequested = resolve(sessionCwd);
  const requested = resolve(sessionRequested, input.cwd);
  if (existing && !existing.isReleased) {
    if (existing.requested !== requested || existing.sessionRequested !== sessionRequested) throw new Error("[SHELL_CWD_CHANGED] cwd changed after preparation.");
    return existing;
  }
  const sessionCanonical = await realpath(sessionRequested);
  const sessionDirectory = await stat(sessionCanonical, { bigint: true });
  if (!sessionDirectory.isDirectory()) throw new Error("[SHELL_CWD_INVALID] Session cwd is not a directory.");
  const canonical = requested === sessionRequested ? sessionCanonical : await realpath(requested);
  const directory = canonical === sessionCanonical ? sessionDirectory : await stat(canonical, { bigint: true });
  if (!directory.isDirectory()) throw new Error("[SHELL_CWD_INVALID] cwd is not a directory.");
  const binding = new ShellCwdBinding(requested, canonical, directory.dev, directory.ino, sessionRequested, sessionCanonical, sessionDirectory.dev, sessionDirectory.ino);
  attachShellCwdBinding(input, binding);
  return binding;
}
