...
export function createAskUserToolDefinition(): ToolDefinition<typeof parameters> {
	return {
		...
		promptSnippet: "Ask one bounded question and wait for an explicit user answer",
		promptGuidelines: ["When a material ambiguity, conflict, or non-inferable preference needs the user's decision, call ask_user and wait for its result; do not ask in prose and continue business tools in the same response."],
		description: "Wait for an explicit user answer about conflicting requirements, preferences or material scope/behavior choices. Do not ask about routine fixable syntax or tool errors. An answer is not shell or file permission. Options are optional; use a single text question when choices would be artificial. Other calls in this response will NOT execute; replan after the answer.",
		async execute(_id, params, signal, onUpdate, ctx) {
			...
			if (!params.options || params.options.length === 0) {
				onUpdate?.({ content: [{ type: "text", text: `等待用户回答：${params.question}` }], details: { status: "waiting_for_user_input" } });
				const text = await ctx.ui.input(params.question, params.description ?? "请直接输入回答", { signal });
				...
				onUpdate?.({ content: [{ type: "text", text: `等待用户选择：${params.question}` }], details: { status: "waiting_for_user_input" } });
