...
export interface CreateCodingAgentHarnessOptions extends Omit<AgentHarnessOptions, "toolContext" | "tools"> {
	...
	/** Path to the JSONL session file exposed to default bash commands as SP_SESSION_FILE. */
	sessionFile?: string;
			...
			createCodingAgentHarnessTool(
				createBashTool<ExecutionToolContext>({
					...
					prepare: async (execution) => {
						...
						execution.env.SP_SESSION_ID = metadata.id;
						execution.env.SP_SESSION_FILE = sessionFile ?? "";
						execution.env.SP_PROVIDER = model.provider;
						execution.env.SP_MODEL = model.id;
						execution.env.SP_REASONING_LEVEL = thinkingLevel;
					},
				}),
				toolContext,
				{
