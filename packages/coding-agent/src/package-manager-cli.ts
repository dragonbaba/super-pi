...
import { Markdown, type MarkdownTheme } from "@super-pi/tui";
...
Update installed packages or model catalogs. Super Pi core self-update is disabled for source checkouts.

Options:
  --self                  Explain how to update this source checkout
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Update installed packages, then show the source update instruction
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Force package updates when supported

Short forms:
  ${APP_NAME} update                Show the source update instruction
  ${APP_NAME} update --all          Update all extensions, then show the source update instruction
...
async function refreshModelCatalogs(agentDir: string): Promise<void> {
	...
	try {
		...
		if (result.errors.size > 0) {
			...
			console.error(
				chalk.yellow(`Warning: Some model catalogs could not be refreshed; cached catalogs remain available: ${details}`),
			);
		}
	} finally {
		clearTimeout(timeout);
	}
	console.log(chalk.green("Model catalog refresh completed"));
}
		...
		switch (options.command) {
			...
			case "update": {
				...
				if (updateTargetIncludesSelf(target)) {
					console.log(
						chalk.yellow(
							"Super Pi core self-update is disabled. Update the source checkout with Git, then run npm install and npm run build:offline.",
						),
					);
				}
				return true;
