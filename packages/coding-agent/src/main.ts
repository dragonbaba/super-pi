...
const EXTENSION_LOAD_FAILURE_HINT = 'Hint: Start without extensions using "superpi -ne".';
...
export async function main(args: string[], options?: MainOptions) {
	...
	const createRuntime: CreateAgentSessionRuntimeFactory = async ({
		...
		const created = await createAgentSessionFromServices({
			...
			toolResultPresentation: settingsManager.getToolResultPresentationOptions(),
		});
	...
	const startupBenchmark = isTruthyEnvFlag(process.env.SP_STARTUP_BENCHMARK);
	if (startupBenchmark && appMode !== "interactive") {
		console.error(chalk.red("Error: SP_STARTUP_BENCHMARK only supports interactive mode"));
		process.exit(1);
	}
