...
const UNCERTAIN_LIFECYCLE = "Blocked an uncertain/uninspectable shell lifecycle before execution.\n[Lifecycle recovery] Actual heredocs are unsupported. For an otherwise authorized diagnostic task, create/edit the script with native file tools, then request separately authorized foreground execution. Each operation keeps its own read, path, permission and lifecycle requirements; missing files need no read. Broader permissions do not resolve parser limits, and changing tools or language cannot legalize denied behavior. No command was executed.";
