...
interface OperationRequest {
  operation: "edit" | "write" | "lsp_fix" | "bash" | "powershell" | "browser_exec" | "delete" | "move";
...
function structuredPermissionBlock(
  ...
  return JSON.stringify({
    ...
    primitives,
    rejectionReason: sanitizePolicyFeedback(options.rejectionReason),
    stateChanged: false,
...
function modelPolicyBlock(request: OperationRequest, mode: SessionPermissionMode): { reason: string; details: unknown } {
  const legacy = {
    ok: false,
    category: "POLICY_BLOCKED",
  ...
  if (!request.diagnostic) {
    return {
      reason: structuredPermissionBlock(request.operation, mode, "unverifiable_target", {
        highRisk: request.highRisk,
        opaqueScript: request.opaqueScript,
        retryable: false,
        primitives: request.primitives,
...
export class SessionPermissionController {
  ...
  async authorizeToolCall(event: ToolCallEventShape, ctx: ExtensionContext): Promise<ToolCallBlock | undefined> {
    if (event.toolName !== "write" && event.toolName !== "edit" && event.toolName !== "lsp_fix" && event.toolName !== "delete" && event.toolName !== "move"
    ...
    if (hasUnverifiableAssessment(request.targetAssessments)) {
      ...
      const block = modelPolicyBlock(request, this.#state.mode);
      return {
        block: true,
  ...
  async #buildRequest(event: ToolCallEventShape, ctx: ExtensionContext): Promise<OperationRequest | undefined> {
    ...
    if (event.toolName === "edit" || event.toolName === "write" || event.toolName === "lsp_fix") {
      ...
      return {
        ...
        pathApproval: protectedAssessment.canonicalTarget && (protectedAssessment.violations.length > 0 || creationPlan)
    ...
    const diagnostic = high?.diagnostic;
    return {
  ...
  #attachPathApproval(event: ToolCallEventShape, request: OperationRequest): void {
    ...
    attachPermissionPathApproval(event.input, {
      ...
      assertCurrent: request.nativePlan || request.creationPlan ? () => {
