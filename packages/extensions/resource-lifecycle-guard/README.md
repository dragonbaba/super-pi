...
The owned-job recipe permits up to 16 literal `curl`, `test`, `true`, `false`, `echo` or `printf` use commands (4096 characters total) between installing cleanup and explicitly killing/waiting on the captured PID. Expansion, PID reassignment and arbitrary runners in this interval are unsupported; do not interpret an unsupported recipe as permission to detach or switch launchers.
