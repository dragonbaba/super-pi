...
| Conditional-predicate or nested-control `cd`, a command substitution after a changing `cd`, `source`/`eval` followed by another command, or a non-exit trap before a later command | Refused before authorization when the later target or shell state cannot be attributed to one cwd. A bounded standalone literal `eval 'printf ...'`, a `trap -p DEBUG` query and an `EXIT` cleanup trap remain eligible for ordinary checks; this does not imply general evaluator or trap state propagation. |
