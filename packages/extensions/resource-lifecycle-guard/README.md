...
| `CDPATH=...; cd workspace && write` and `export`/`declare`/`typeset`/`readonly CDPATH=...; cd ...` | Refused before authorization and spawn because the earlier parent-shell assignment changes `cd` lookup. An assignment without a later `cd` remains eligible for ordinary permission checks. |
