...
Update-safe local fork of `@narumitw/pi-goal@0.49.5` for Pi 0.84.1. It preserves the upstream `/goal`, `goal_complete`, and `goal_blocked` interfaces while preventing long-running Goal context amplification:

- automatic continuation messages are compact and do not persist another copy of the objective and rules;
- legacy verbose continuation messages are reduced only in the provider context copy, without rewriting Session history;
- full kickoff/resume/edit prompts are not duplicated into the same request's system prompt;
- exhausted context-overflow compaction retries remain active and create a compact continuation boundary instead of becoming `blocked`;
- resume rebases accounting so assistant usage accumulated while paused, blocked, or usage-limited is never charged retroactively to the Goal;
- the local default and `pi-goal.json` use Unlimited automatic turns; explicit `--tokens` budgets remain opt-in;
- terminal tool metadata stays concise because the active Goal system context already carries the complete completion and blocker policy.

The ordinary no-progress guard remains enabled. Network, authentication, quota, and non-context terminal failures retain upstream fail-safe behavior.
...
- Keeps retryable provider interruptions and Pi compaction retries active without enqueueing duplicate goal continuations while Pi retries. This fork turns an exhausted context-overflow retry into a compact continuation boundary; other exhausted provider failures remain fail-safe `blocked` states.
