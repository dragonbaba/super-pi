...
function cloneUserMessagesForDryRun<T>(messages: T[]): T[] {
	const cloned = messages.slice();
	for (let index = 0; index < cloned.length; index += 1) {
		const message = cloned[index];
		if (!message || typeof message !== "object" || Reflect.get(message, "role") !== "user") {
			continue;
		}
		...
		cloned[index] = { ...message, content: parts };
	}
	return cloned;
}
...
export function registerGoalLifecycle(
	...
	pi.on("context", (event, ctx) => {
		...
		for (let index = 0; index < messages.length; index += 1) {
			if (runtime.keepBudgetWrapUpMessage(messages[index])) continue;
			const filtered = messages.slice(0, index);
			for (let tailIndex = index + 1; tailIndex < messages.length; tailIndex += 1) {
				const message = messages[tailIndex];
				if (runtime.keepBudgetWrapUpMessage(message)) filtered.push(message);
			}
			messages = filtered;
			break;
		}
		const compactionInput = dryRun ? cloneUserMessagesForDryRun(messages) : messages;
		const historyCompacted = runtime.compactLegacyContinuationHistory(compactionInput);
		if (historyCompacted) messages = compactionInput;
		if (
			!dryRun &&
			runtime.activeGoal?.status === "paused" &&
			runtime.guardAbortGoalId === runtime.activeGoal.id
		...
		if (historyCompacted || messages !== event.messages) return { messages };
	});
	...
	pi.on("before_agent_start", (event, ctx) => {
		...
		// Kickoff, resume, and edit are durable user messages containing the complete
		// objective and rules. Automatic continuations rely on that append-only history
		// instead of rebuilding a changing Goal system-prompt suffix on every turn.
	});
