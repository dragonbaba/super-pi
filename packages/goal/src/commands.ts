import {
	...
	currentTokenTotal,
	formatTokenCount,
	rebaseGoalUsageAfterStoppedInterval,
} from "./accounting.js";
import { validateObjective } from "./command.js";
import { safeGoalMenuText } from "./goal-text.js";
...
export class GoalCommandController {
		...
		if (!sent) {
			...
			if (this.runtime.activeGoal?.id === startedGoal.id) {
				...
				if (existingGoal) {
					...
					else rebaseGoalUsageAfterStoppedInterval(existingGoal, ctx);
	...
	async resumeGoal(ctx: StatusContext) {
		...
		rebaseGoalUsageAfterStoppedInterval(this.runtime.activeGoal, ctx);
