...
export function buildContinuePrompt(goal: GoalPromptContext, marker: string) {
	return `Continue the active /goal from the authoritative current state. This is automatic continuation #${goal.iteration}. The full objective and completion rules remain active in the durable Goal instruction history; do not repeat or narrow them. If every requirement is verified, call goal_complete with goal_id ${escapeXmlText(goal.id)}. Otherwise keep working.\n\n${continuationMarkerComment(marker)}`;
}
