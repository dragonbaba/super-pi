...
const CONTINUATION_MARKER_OPEN = "<!-- pi-goal-continuation:";
const LEGACY_GOAL_OBJECTIVE_TAG = "<goal_objective>";
const COMPACTED_CONTINUATION_TEXT =
	"[Earlier automatic /goal continuation; the active Goal system context retains the objective and completion rules.]";
...
export class GoalRuntime {
	...
	compactLegacyContinuationHistory(messages: unknown[]) {
		const goalId = this.activeGoal?.id;
		if (!goalId) return false;
		const ownedMarker = `${CONTINUATION_MARKER_OPEN}${goalId}:`;
		let changed = false;
		for (const message of messages) {
			...
			if (typeof candidate.content === "string") {
				if (isLegacyOwnedContinuation(candidate.content, ownedMarker)) {
					candidate.content = COMPACTED_CONTINUATION_TEXT;
					changed = true;
				}
				continue;
			}
			...
			for (const part of candidate.content) {
				...
				if (typeof text !== "string" || !isLegacyOwnedContinuation(text, ownedMarker)) continue;
				Reflect.set(part, "text", COMPACTED_CONTINUATION_TEXT);
				changed = true;
			}
		}
		return changed;
	...
	finalizeSettledRecovery(ctx: StatusContext) {
		...
		if (recovery.kind === "compaction_retry") {
			this.persistGoal(goal);
			this.updateStatus(ctx, goal);
			this.requestContinuation(goal);
			ctx.ui.notify(
				`Goal context recovery will continue with a compact prompt${details}.`,
				"warning",
			);
			return true;
		}
...
function isLegacyOwnedContinuation(text: string, ownedMarker: string) {
	return text.includes(ownedMarker) && text.includes(LEGACY_GOAL_OBJECTIVE_TAG);
}
