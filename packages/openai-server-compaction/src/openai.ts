...
export function modelKey(model: ModelLike): string {
  return `${unknownText(model.provider)}:${unknownText(model.api)}:${unknownText(model.id)}`;
}
...
export function extractResponsesServiceTier(payload: unknown): RemoteCompactionServiceTier | undefined {
  if (!isRecord(payload)) return undefined;
  switch (payload.service_tier) {
    case "auto":
    case "default":
    case "flex":
    case "scale":
