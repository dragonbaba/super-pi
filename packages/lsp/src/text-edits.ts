...
export function collectWorkspaceEdits(edit: WorkspaceEdit | undefined, uri: string): readonly LspTextEdit[] {
	if (!edit) return EMPTY_READONLY_ARRAY;
	if (edit.documentChanges) {
		const matchingEdits: LspTextEdit[] = [];
		for (const change of edit.documentChanges) {
			if (change.textDocument?.uri === uri && change.edits) matchingEdits.push(...change.edits);
		}
		return matchingEdits.length > 0 ? matchingEdits : EMPTY_READONLY_ARRAY;
	}

	return edit.changes?.[uri] ?? EMPTY_READONLY_ARRAY;
}
