...
/** Synchronous SHA-256 for runtimes without Node/Bun built-in crypto. */
function sha256Portable(bytes: Uint8Array): string {
	const paddedLength = Math.ceil((bytes.byteLength + 9) / 64) * 64;
	const padded = new Uint8Array(paddedLength);
	padded.set(bytes);
	padded[bytes.byteLength] = 0x80;
	const bitLength = BigInt(bytes.byteLength) * 8n;
	const paddedView = new DataView(padded.buffer, padded.byteOffset, padded.byteLength);
...
export function sha256Utf8(value: string): string {
	if (typeof process !== "undefined" && (process.versions?.node || process.versions?.bun)) {
		...
		if (crypto) return crypto.createHash("sha256").update(value).digest("hex");
	}
	return sha256Portable(UTF8_ENCODER.encode(value));
}
