 ...
 * - z.ai: "Prompt too long", or silent overflow via usage.input > contextWindow
...
export function isContextOverflow(message: AssistantMessage, contextWindow?: number): boolean {
	...
	if (message.stopReason === "error" && message.errorMessage) {
		...
		if (!isNonOverflow && (
			...
			(message.provider === "cerebras" && CEREBRAS_BODYLESS_OVERFLOW_PATTERN.test(message.errorMessage))
		)) {
			return true;
		}
	}
