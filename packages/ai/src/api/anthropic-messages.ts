 ...
 * Defaults to "short" and uses SP_CACHE_RETENTION for backward compatibility.
 */
function resolveCacheRetention(cacheRetention?: CacheRetention, env?: ProviderEnv): CacheRetention {
	if (cacheRetention) {
		return cacheRetention;
	}
	if (getProviderEnvValue("SP_CACHE_RETENTION", env) === "long") {
		return "long";
	}
	return "short";
}
			...
			for await (const event of iterateAnthropicEvents(response, options?.signal)) {
				if (event.type === "message_start") {
					...
					if (event.content_block.type === "text") {
						...
						blockPositions[event.index] = contentIndex;
					...
					if (event.delta.type === "text_delta") {
						const index = blockPositions[event.index] ?? -1;
