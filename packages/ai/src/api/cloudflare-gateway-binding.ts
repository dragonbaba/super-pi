...
export function createGatewayBindingFetch(options: GatewayBindingFetchOptions): FetchFunction {
	...
	const gatewayBinding = binding.gateway(gateway);
	...
	return async (input: FetchInput, init?: RequestInit): Promise<Response> => {
		...
		if (method !== "POST") return throwUnexpressibleGatewayRequest(method, url, "only POST is supported");

		const rest = parsed.pathname.slice(basePath.length);
		const slash = rest.indexOf("/");
		if (slash <= 0) {
			return throwUnexpressibleGatewayRequest(method, url, "missing provider/endpoint path");
		}
		...
		try {
			...
			return throwUnexpressibleGatewayRequest(method, url, "non-JSON body");
		}
		if (query === undefined) {
			return throwUnexpressibleGatewayRequest(method, url, "missing body");
		}
		...
		return gatewayBinding.run({ provider, endpoint, headers, query }, signal ? { signal } : undefined);
	};
}

function throwUnexpressibleGatewayRequest(method: string, url: string, reason: string): never {
	throw new Error(
...
async function readBodyText(request: Request | undefined, init?: RequestInit): Promise<string | undefined> {
	...
	if (body instanceof Uint8Array) return UTF8_DECODER.decode(body);
	if (body instanceof ArrayBuffer) return UTF8_DECODER.decode(new Uint8Array(body));
...
function collectHeaders(request: Request | undefined, init?: RequestInit): Record<string, string> {
	...
	if (headers === undefined) {
		if (request) {
			for (const [key, value] of request.headers) addGatewayHeader(result, key, value);
		}
	} else if (headers instanceof Headers) {
		for (const [key, value] of headers) addGatewayHeader(result, key, value);
		...
		for (let index = 0; index < headerNames.length; index++) {
			...
			if (value !== undefined) addGatewayHeader(result, key, value);
		}
	}
	return result;
}

function addGatewayHeader(result: Record<string, string>, key: string, value: string): void {
	const name = key.toLowerCase();
	if (!STRIP_HEADERS.has(name)) setOwnProperty(result, name, value);
}
