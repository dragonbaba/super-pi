...
```bash
npm install @super-pi/ai
```

TypeBox exports are re-exported from `@super-pi/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@super-pi/ai';
import { builtinModels } from '@super-pi/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@super-pi/ai/providers/anthropic';
import { openaiProvider } from '@super-pi/ai/providers/openai';
import { openrouterProvider } from '@super-pi/ai/providers/openrouter';
import { amazonBedrockProvider } from '@super-pi/ai/providers/amazon-bedrock';
...
```typescript
import { hasApi } from '@super-pi/ai';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@super-pi/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@super-pi/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@super-pi/ai';
...
```typescript
import { validateToolCall, type Tool } from '@super-pi/ai';
...
```typescript
import { builtinImagesModels } from '@super-pi/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@super-pi/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@super-pi/ai';
import { openAICompletionsApi } from '@super-pi/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@super-pi/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@super-pi/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@super-pi/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@super-pi/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@super-pi/ai';
import { anthropicProvider } from '@super-pi/ai/providers/anthropic';
import { openaiProvider } from '@super-pi/ai/providers/openai';
import { googleProvider } from '@super-pi/ai/providers/google';
...
- `@super-pi/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@super-pi/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@super-pi/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@super-pi/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@super-pi/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@super-pi/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@super-pi/ai/bedrock-provider';
...
Pass `env` in stream options to scope provider configuration to a request. Values in `env` are used before process environment variables for provider auth and configuration such as Cloudflare account IDs, Azure OpenAI settings, Vertex project/location, Bedrock settings, `SP_CACHE_RETENTION`, and `HTTP_PROXY`/`HTTPS_PROXY`.
...
```bash
npx @super-pi/ai login              # interactive provider selection
npx @super-pi/ai login anthropic    # login to specific provider
npx @super-pi/ai list               # list available providers
```
...
Built-in login and refresh flows are private provider implementations. Use provider-owned `OAuthAuth`, which composes with `CredentialStore` and gets locked auto-refresh through `Models`. The `@super-pi/ai/oauth` entry point retains only type declarations required by coding-agent extension OAuth compatibility.
...
```typescript
// Before
import { getModel, complete } from '@super-pi/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@super-pi/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@super-pi/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@super-pi/ai`.
