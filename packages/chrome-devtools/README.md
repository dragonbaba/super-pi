# 🌐 sp-chrome-devtools — Chrome DevTools Tools for Pi Agents

[![npm](https://img.shields.io/npm/v/@super-pi/chrome-devtools)](https://www.npmjs.com/package/@super-pi/chrome-devtools) [![Pi extension](https://img.shields.io/badge/Pi-extension-blue)](https://pi.dev) [![License: MIT](https://img.shields.io/badge/license-MIT-green.svg)](./LICENSE)

`@super-pi/chrome-devtools` is a native [Pi coding agent](https://pi.dev) extension that exposes Chrome DevTools Protocol (CDP) automation as Pi tools.
...
- Uses `@super-pi/tui-kit` for width-safe menus and individual tool selection.
- Persists the selected Chrome DevTools tools across Pi restarts.

## 📦 Install

```bash
superpi install ./packages/chrome-devtools
```

Try without installing permanently:

```bash
superpi -e ./packages/chrome-devtools
```
...
When `SP_CHROME_DEVTOOLS_PORT` is not set, auto-launch uses Chrome's dynamic DevTools port mode
(`--remote-debugging-port=0`) and reads `DevToolsActivePort` from the temp profile. This avoids
forcing every Pi session onto port `9222`. If you set `SP_CHROME_DEVTOOLS_PORT` to a valid port
...
When `SP_CHROME_DEVTOOLS_BROWSER` is set, that executable is the only auto-launch candidate; a
...
```bash
SP_CHROME_DEVTOOLS_AUTO_LAUNCH=0 superpi -e ./packages/chrome-devtools
```

Force a browser executable or endpoint if needed:

```bash
SP_CHROME_DEVTOOLS_BROWSER=/usr/bin/brave-browser superpi -e ./packages/chrome-devtools
SP_CHROME_DEVTOOLS_HOST=127.0.0.1 SP_CHROME_DEVTOOLS_PORT=9223 superpi -e ./packages/chrome-devtools
```
...
```bash
google-chrome --remote-debugging-port=9222 --user-data-dir=/tmp/sp-chrome-devtools
```
...
/Applications/Google\ Chrome.app/Contents/MacOS/Google\ Chrome \
  ...
  --user-data-dir=/tmp/sp-chrome-devtools
```
...
```text
/tmp/sp-chrome-devtools-screenshot-<uuid>.png
```
...
```text
${SP_CODING_AGENT_DIR:-~/.sp/agent}/config/sp-chrome-devtools.json
```
...
Compatibility: older versions used `sp-chrome-devtools-settings.json`. A legacy-only file remains
readable with a warning and is never modified automatically; rename it to
`sp-chrome-devtools.json`. The first subsequent settings save writes the canonical file. If both
files exist, `sp-chrome-devtools.json` wins and the legacy file is ignored. The legacy
