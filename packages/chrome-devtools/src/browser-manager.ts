...
export async function shutdownManagedBrowser(
	...
	if (!managedBrowser.exited) {
		...
		try {
			await waitForManagedBrowserExit(managedBrowser, BROWSER_SHUTDOWN_WAIT_MS);
		} catch {
	...
	try {
		...
		throw new Error(`Failed to remove owned Chrome profile "${managedBrowser.userDataDir}": ${formatError(error)}`);
	}
...
function killManagedBrowserProcess(managedBrowser: ManagedBrowser, signal?: NodeJS.Signals) {
	const pid = managedBrowser.process.pid;
	if (process.platform === "win32" && pid && pid > 0) {
		...
		if (result.error) throw new Error(`System32 taskkill failed for Chrome PID ${pid}: ${result.error.message}`);
		if (result.status !== 0 && processIsAlive(pid)) {
			throw new Error(`System32 taskkill exited with status ${result.status ?? "unknown"}; Chrome PID ${pid} is still running`);
		}
		return;
	}
	try {
		...
		if (pid && processIsAlive(pid)) throw new Error(`Failed to terminate owned Chrome PID ${pid}`);
	}
}
...
export function endpointSourceLabel() {
	const hostSource = state.hostConfigured ? "SP_CHROME_DEVTOOLS_HOST" : "default host";
	const portSource = state.portConfigured ? "SP_CHROME_DEVTOOLS_PORT" : "default/dynamic port";
...
export function chromeLaunchCommand() {
	...
	const dataDir =
		process.platform === "win32" ? "%TEMP%\\sp-chrome-devtools" : "/tmp/sp-chrome-devtools";
...
export function endpointConfigHint() {
	return "Set SP_CHROME_DEVTOOLS_HOST and SP_CHROME_DEVTOOLS_PORT for a manual endpoint, SP_CHROME_DEVTOOLS_BROWSER to choose an executable, or SP_CHROME_DEVTOOLS_AUTO_LAUNCH=0 to disable auto-launch.";
}
...
function explicitBrowserCandidateDefinition(): BrowserCandidateDefinition[] {
	...
	return [
		{ label: "SP_CHROME_DEVTOOLS_BROWSER", executable: state.browserExecutable, source: "env" },
	];
}
