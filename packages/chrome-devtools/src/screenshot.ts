...
export async function saveScreenshot(
	...
	if (pngBytes.byteLength > MAX_SCREENSHOT_BYTES) {
		throw new Error(`Chrome screenshot exceeds ${MAX_SCREENSHOT_BYTES} bytes; capture the viewport or reduce page dimensions.`);
	}
...
export function resolveScreenshotPath(
	...
	if (savePath === undefined) {
		return {
			path: join(tempRoot, `sp-chrome-devtools-screenshot-${randomUUID()}.png`),
