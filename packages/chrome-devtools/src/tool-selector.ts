...
import {
	...
	reconcileChromeDevtoolsTools,
} from "./active-tool-policy.js";
import {
...
export function applyChromeDevtoolsTools(
	pi: ExtensionAPI,
	selectedTools: readonly ChromeDevToolsToolName[],
	options: { preservePlanModeSelection?: boolean } = {},
) {
	const activeToolNames = pi.getActiveTools();
	const next = reconcileChromeDevtoolsTools(
		activeToolNames,
		selectedTools,
		options.preservePlanModeSelection === true,
	);
	if (next !== undefined) pi.setActiveTools(next);
}
...
function getToolStatusSummary(pi: ExtensionAPI): ToolStatusSummary {
	...
	const activeNonChromeToolCount = Array.from(activeToolNames).filter(
		(name) => !CHROME_DEVTOOLS_TOOL_NAME_SET.has(name),
	).length;
	const runtimeStatus =
